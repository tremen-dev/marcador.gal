...
## Resumen
- **LO ÚLTIMO (2026-09-01): LA SÉPTIMA VERIFICACIÓN SALIÓ GREEN, y la spec se
  cierra.** Ver *«Veredicto del verificador — séptima vuelta: GREEN»*, al final.
  **Los tres gates verdes y reproducidos** (`lint exit=0`, `npm test` **777/777**,
  `npm run test:db` **144/144** contra Postgres real; `DATABASE_URL_TEST`
  disponible, **ningún criterio UNMET**). **F-SPEC-008-V37 está CERRADO**, y lo
  comprobé escribiendo yo los ficheros: `.js`, `.jsx` y `.mjs` bajo las raíces son
  **rojos por el mismo caso y con el mismo mensaje** que su gemelo `.ts`, el
  `.cjs` muere **por cuatro caminos**, los cuatro fuera de las raíces ponen rojo la
  cobertura, y **la ruta viva se reprodujo entera**: `next build` compila
  `src/app/(gl)/w7probe/route.js` (`ƒ /w7probe` en la tabla de rutas de
  producción), `next start` la sirve (`curl` → `w7probe`), y hoy es **roja** en
  `architecture` 3 y 17. **La exclusión gobierna las dos listas y no pude
  silenciar una sola** (cinco mutaciones propias, las cinco muerden).
  **`CAPABILITY_NAMES`, `resolveModule()`, `.gitignore`, `.oxlintrc.json`,
  `ALLOWED_PACKAGES`, `src/` y `migrations/` intactos.** **N4 sobrevive
  (189/189)**, y **CA-7 y CA-14 siguen ciertos** (9 rojos y 8 rojos). **17 filas
  ✅, cinco ⚠️, ninguna ❌.** **CA-2 se cierra en ⚠️**, que es su techo firmado.
  Dos findings nuevos, **ninguno bloqueante y los dos a SPEC-009**:
  **F-SPEC-008-V39** (la exclusión de `docs/diseno/` es por directorio, así que
  también silencia `.ts`/`.tsx`, que ayer eran rojos — **medido**, y con un
  `import` desde una ruta desplegada entra en el bundle de producción) y
  **F-SPEC-008-V40** (**hay una sexta lista de extensiones**, en los guardianes de
  i18n de `tests/site/`, y es **la única de las seis que falla ABIERTO**). La spec
  pasa a **`hecho`**.
- **LO ÚLTIMO (2026-09-01): SÉPTIMA VUELTA DE IMPLEMENTACIÓN, y es un diff con
  motivo.** Ver *«Séptima vuelta — la declaración dice la verdad sobre qué compila
  y ejecuta este proyecto»*, al final. Cierra **F-SPEC-008-V37 y solo eso**:
  `SCAN_EXTENSIONS` declara ahora **ocho** sufijos —los cuatro de TypeScript más
  **`.js`, `.jsx`, `.mjs` y `.cjs`, cada uno con su motivo escrito**— y
  `docs/diseno/` entra en `SCAN_EXCLUSIONS` **como decisión declarada con su
  motivo**, citando el precedente del gate de calidad (`.oxlintrc.json` ya la
  ignora, commit `5b632df`). Las exclusiones pasan a gobernar **las dos listas**,
  no una. Dos casos nuevos (`architecture` **2h** y **2i**), **777/777**, **91/91**
  en `tests/polite`, los tres gates verdes con `DATABASE_URL_TEST` disponible
  (**ningún criterio UNMET**), y **once mutaciones**: nueve muertas, **T9 y N4
  vivas porque deben**. **`src/` sin una línea de diff**, y **medido** que
  ensanchar la declaración **no mueve ni un fichero** de las listas existentes
  (89 → 89, nada entra y nada sale). `CAPABILITY_NAMES`, `.gitignore`,
  `.oxlintrc.json` y `resolveModule()` **no se han tocado**: F-SPEC-008-V34, V35 y
  V36 siguen abiertos a propósito y son de SPEC-009. Salvedad nueva:
  **F-SPEC-008-V38** (lo que la exclusión de `docs/diseno/` cuesta, medido;
  destino SPEC-009). Estado: `en-revision`. **CA-2 sigue teniendo ⚠️ como estado
  máximo alcanzable.**
- **LO ANTERIOR (2026-09-01): LA SEXTA VERIFICACIÓN SALIÓ RED, y por UN SOLO
  finding que no es ninguno de los dos mudados a SPEC-009.** Ver *«Veredicto del
  verificador — sexta vuelta: RED»*, al final. **Los tres gates verdes y
  reproducidos** (`lint exit=0`, `npm test` **775/775**, `npm run test:db`
  **144/144** contra Postgres real; `DATABASE_URL_TEST` disponible, **ningún
  criterio UNMET**). **Trece CA ✅**, cuatro ⚠️ y **uno ❌**. **El mecanismo de la
  sexta vuelta es correcto y F-SPEC-008-V33 está cerrado para lo que nombra**: un
  `.mts` y un `.cts` escritos por el verificador son rojos **por el mismo caso y
  con el mismo mensaje** que su gemelo `.ts`, los de fuera de las raíces ponen
  rojo la cobertura, **las dos listas no se pueden mover por separado** (cinco
  mutaciones, las cinco muerden), **`CAPABILITY_NAMES` está intacto**, `.gitignore`,
  `src/` y `migrations/` tienen **diff vacío**, **N4 sobrevive (187/187)** y el
  residuo de V34 y V35 se **reprodujo y es cierto**. **Lo que bloquea es que hay
  una QUINTA forma y la declaración no la cubre: JavaScript.** `SCAN_EXTENSIONS`
  se paró en la familia de TypeScript, así que `.js`, `.jsx`, `.mjs` y `.cjs`
  quedan fuera **por efecto colateral y no por decisión declarada** — que es
  exactamente lo que CA-2.6 garantiza que no pasa. Medido:
  `src/app/(gl)/vprobe/route.js` con `node:child_process` lo **compila
  `next build`** (`ƒ /vprobe`, tabla de rutas de producción), lo **sirve
  `next start`**, deja los cuatro números en verde, **se commitea con un `git add`
  normal** y **ninguna de las dos listas lo ve**. Es **F-SPEC-008-V37**, y es de
  **esta** spec: cerrarlo es lo que CA-2.6.5 llama *«un diff con motivo… nunca un
  arbitraje»*. La spec vuelve a `en-progreso`. **CA-2 sigue teniendo ⚠️ como
  estado máximo alcanzable.**
- **LO ANTERIOR (2026-09-01): SEXTA VUELTA DE IMPLEMENTACIÓN, y es corta a
...
- **LO ANTERIOR: la QUINTA VERIFICACIÓN salió RED** — *«Veredicto del verificador —
  ...
  **144/144** contra Postgres real, `DATABASE_URL_TEST` disponible: **ningún
  criterio UNMET**). **Trece CA quedan ✅**, cuatro ⚠️ y **uno ❌**. El lector del
  compilador es correcto y lo comprobé rompiendo: la **novena** evasión muere en
  sus tres escrituras, el fichero bajo `src/**/robots/` muere **dos veces**, la
  octava muere **en las dos mitades** cuando se engancha donde pasa el camino
  conducido, R9..R13 muerden, y **N4 sobrevive porque debe** (184/184). **Lo que
  bloquea sigue siendo CA-2, y son la DÉCIMA y la UNDÉCIMA evasión, las dos
  medidas mandando una petición real sin User-Agent, sin `robots.txt` y sin
  turno.** La décima: **`src/ingest/door.mts`** —un fichero `.mts` bajo una raíz
  del escaneo— **no está ni en la lista que se lee ni en la de cobertura**
  (`scannedSources()` filtra por `.ts`/`.tsx`; el pathspec de `versionedSources()`
  es `*.ts`/`*.tsx`), deja `lint exit=0`, `npm test` **772/772** y `tests/polite`
  **86/86**, y **se commitea con un `git add` normal**; el mismo fichero renombrado
  a `.ts` es **rojo** en `architecture` 3 (F-SPEC-008-V33). La undécima:
  **`process.getBuiltinModule('node:child_process')`**, sin ningún `import` y sin
  ningún identificador de `CAPABILITY_NAMES`, en un `.ts` normal, escaneado y
  alcanzable — 772/772 y `lint exit=0` (F-SPEC-008-V34). La spec vuelve a
  `en-progreso`.
- Fase anterior: **en-revision** (tras la QUINTA vuelta de implementación).
- **LA QUINTA VUELTA está hecha** — *«Quinta vuelta — el lector deja
...
## Veredicto del verificador — quinta vuelta: RED (2026-09-01, `sdd-verificador`)

**RED.** Trece CA quedan ✅, cuatro ⚠️ y **uno ❌**. El lector del compilador es
correcto, y lo comprobé rompiendo y no leyendo: **la novena evasión muere en sus
tres escrituras sobre un fichero real**, el fichero escondido bajo
`src/**/robots/` muere **dos veces**, la octava repuesta muere **en las dos
mitades** cuando se engancha donde pasa el camino conducido, R7..R13 muerden
todas, y **N4 sobrevive porque debe**. `moduleSpecifiers` y
`withoutImportStatements` están **borrados**, no envueltos. La obligación 1 —un
solo lector— se cumple, y el caso 17 pregunta al lector en vez de a su propio
patrón.

**Pero el agujero no se cerró: se movió.** Dejó de estar en el lector y pasó a
estar en **la lista de ficheros** y en **CA-2.4**, y hay **dos evasiones nuevas,
las dos medidas mandando una petición real sin User-Agent, sin `robots.txt` y sin
turno**:

- **La décima (F-SPEC-008-V33)**: `src/ingest/door.mts`. Un `.mts` bajo una raíz
  del escaneo **no está ni en la lista que se lee ni en la de cobertura**, y
  **se commitea con un `git add` normal**. Es estrictamente peor que
  F-SPEC-008-V28, que necesitaba `git add -f`.
- **La undécima (F-SPEC-008-V34)**: `process.getBuiltinModule('node:child_process')`
  en un `.ts` corriente, escaneado y alcanzable, **sin ningún `import`** y sin
  ningún identificador de la lista de CA-2.4.

Ninguna de las dos cae en un residuo declarado, y las dos contradicen una frase
que un CA afirma con esas palabras.
...
> vitest run
 ...
 RUN  v4.1.11 /Users/albertofojo/src/tremen-dev/marcador.gal/.claude/worktrees/agent-adfe38bbbff4e5714

 Test Files  81 passed (81)
      Tests  772 passed (772)
Type Errors  no errors
   Start at  20:08:16
   Duration  3.81s (transform 1.86s, setup 0ms, import 8.32s, tests 11.81s, environment 4ms, typecheck 141ms)
```
...
> node --env-file-if-exists=.env.local ./node_modules/vitest/vitest.mjs run --config vitest.integration.config.ts tests/db
 ...
 Test Files  8 passed (8)
      Tests  144 passed (144)
   Start at  20:08:30
   Duration  49.47s (transform 77ms, setup 0ms, import 274ms, tests 48.59s, environment 0ms)
```

Y CA-3 aparte: `npx vitest run tests/mirror tests/site tests/docs` → **48
ficheros / 347 casos** en verde.

*Anotación de estabilidad, sin cobrar: en una de las quince ejecuciones de
`npm test` de esta verificación —con una mutación puesta— salió `1 failed`, y no
volvió a reproducirse en las ocho ejecuciones limpias posteriores. No lo nombro
como finding porque no pude aislarlo; queda dicho por si vuelve.*

### Lo irreversible, primero

- **Ningún HTML real de terceros versionado.** `git diff --name-status main` no
  añade ni un `.html`; `tests/fixtures/ceroacero.ts` es sintético y lo dice en su
  cabecera. **ADR-009 §3 se respeta.**
- **Ningún symlink versionado** (`git ls-files -s | awk '$1=="120000"'` → vacío),
  **ningún `node_modules`** (`git ls-files | grep -c node_modules` → 0) y ningún
  `.env` salvo `.env.example`. **F-SPEC-008-V22 confirmado cerrado.**
- **Ningún fichero de mutación olvidado**: `git status --porcelain` vacío al
  terminar, `src/ingest/`, `src/db/` y `robots/` sin restos, y `.gitignore` sin
  tocar (`git diff 425fe05 HEAD -- src migrations .gitignore` → **vacío**).
- **Ninguna petición a un tercero en toda esta verificación.** Lo único que salió
  fue a `127.0.0.1` (sondas del scratchpad, servidor local propio) y a la rama de
  test de Neon.
...
`tests/mirror`, `tests/site` y `tests/docs`: **44 ficheros en cada referencia** y
**una sola línea de diferencia**, `tests/mirror/user-agent.test.ts` **15 → 16**,
que sube. **Ningún caso borrado.** Sobre las suites de SPEC-001 (`tests/model`,
`tests/raw`, `tests/db`, `tests/types`): la única diferencia es el fichero nuevo
`tests/db/rate-limit.test.ts:7`.

### La décima evasión: la extensión no está en ninguna de las dos listas

**El mecanismo.** `scannedSources()` (`tests/polite/support/capability.ts`) sólo
recoge `path.endsWith('.ts') || path.endsWith('.tsx')`, y `versionedSources()`
le pide a `git` el pathspec `'*.ts', '*.tsx'`. **`.mts` y `.cts` no casan con
ninguno de los dos**: `'src/ingest/door.mts'.endsWith('.ts')` es `false`, y
`git ls-files '*.ts'` tampoco lo lista. Así que el fichero **no se lee** (CA-2.3,
CA-2.4) y **el caso de cobertura tampoco lo ve** (CA-2.6 caso 1), que es
justamente el caso que hace que «quedar fuera sea una decisión declarada».

**La evasión, medida.** `src/ingest/door.mts`:
...
enganchado a `src/ingest/adapter.ts` —que es un `ENTRY_POINT`— con
`import { preflight } from './door.mts';` y un `export const probe = …`:

| Variante | Diferencia | `npm run lint` | `npm test` | `tests/polite` |
|---|---|---|---|---|
| **Evasión** | el fichero se llama `door.mts` | **exit=0** | **772/772** | **86/86** |
| **Control** | el MISMO fichero renombrado a `door.ts` | — | — | **ROJO**, `architecture` 3: `src/db/… node:child_process is not a declared package entry` → aquí `src/ingest/door.ts: node:child_process is not a declared package entry` |

**La única diferencia entre verde y rojo es una letra en el nombre del fichero.**

**Y es committeable, que es lo que la separa de F-SPEC-008-V28.**

```
$ git status --short
 M src/ingest/adapter.ts
?? src/ingest/door.mts
$ git check-ignore -v src/ingest/door.mts
(nada: exit 1)
$ git ls-files --cached --others --exclude-standard '*.ts' '*.tsx' | grep -c door
0
```

`git status` lo enseña y un `git add` normal lo mete en el commit: **este fichero
llega a producción**. El de V28 no podía sin `git add -f`.

**Que manda un paquete de verdad y sin nada de RN-11**, contra un servidor local
propio en `127.0.0.1` —nunca un tercero—:

```
peticiones recibidas por el servidor local: 1
[{"url":"/edicion/galicia-preferente-autonomica-grupo-1-26-27/222309","ua":"(NINGUNO)"}]
pidio robots.txt antes: false
```

**Por qué no es un residuo declarado.** CA-2.6 escribe el suyo así: *«un fichero
fuera de todas las raíces sigue sin leerse, y eso es deliberado»*. Éste **está
bajo una raíz** y no se lee. Y el encabezado del criterio dice *«El escaneo cubre
todo el código del repositorio»*. Ninguna de las cinco cosas que CA-2.3 «NO
promete» ni la que CA-2.6 «NO promete» nombran la extensión. ADR-016 §6 —ya
**aprobado**— obliga a declarar dentro del CA lo que el mecanismo no alcanza, y
esto no está declarado en ninguna parte.

### La undécima evasión: `process` no es un identificador de la lista

**El mecanismo.** CA-2.4 afirma, con estas palabras: *«Sin un `import`,
ECMAScript da exactamente tres maneras de alcanzar una capacidad: el objeto
global, un identificador global desnudo, y `eval`/`Function`. […] Es un conjunto
cerrado por el lenguaje, no por nosotros.»* Y `capabilityOffences` lo mecaniza
con `CAPABILITY_NAMES`, **nueve nombres**: `globalThis`, `fetch`,
`XMLHttpRequest`, `WebSocket`, `EventSource`, `navigator`, `eval`, `Function`,
`require`.

**El conjunto no está cerrado por el lenguaje: está cerrado por esos nueve
nombres.** Node publica **`process`**, que no es un global de ECMAScript sino del
anfitrión, y desde Node 22 `process.getBuiltinModule(id)` es API **estable** que
devuelve cualquier módulo interno **sin `import`**.

**La evasión, medida.** `src/ingest/door11.ts` —un `.ts` corriente, bajo una
raíz, escaneado por CA-2.3/CA-2.4/CA-2.5/CA-2.6, alcanzable desde
`ENTRY_POINTS`—:
...
| | `npm run lint` | `npm test` | `tests/polite` | `git status` |
|---|---|---|---|---|
| **Evasión** | **exit=0** | **772/772** | **86/86** | lo enseña; se commitea sin `-f` |

No hay ningún `import` que CA-2.3 pueda juzgar; `importOffences` no encuentra ni
un especificador. No hay ningún nombre de `CAPABILITY_NAMES`; `capabilityOffences`
devuelve vacío. Y `node:child_process` **no pasa por
`net.Socket.prototype.connect`**, así que CA-2.1 tampoco la ve — es exactamente
el residuo cuyo *«único cierre es el estático»*, y el estático vuelve a no
cerrar.

**Medida contra un servidor local propio:**
...
**Y lo que la hace una decisión y no un arreglo**: taparla añadiendo `process` a
`CAPABILITY_NAMES` es **añadir un nombre a una lista negra**, que es literalmente
lo que ADR-016 §3.1 y la obligación 2 de CA-2.3 prohíben —*«no se añade ningún
nombre a ninguna lista negra: se borra la lista negra»*—. **`CAPABILITY_NAMES`
ES una lista negra de nueve nombres viva dentro del criterio que vino a abolir
las listas negras**, un criterio más allá de donde ADR-016 la cazó la primera
vez. Ver *Lo que necesita decisión humana* §1.

### El tercer hueco, más estrecho: fuera de las raíces, `git` sigue decidiendo

**F-SPEC-008-V35.** `robots/side.ts` **en la raíz del repositorio** —hay un
directorio `robots/` ahí, y `.gitignore:17` (`**/robots/*`) esconde su
contenido—, importado desde `src/ingest/adapter.ts` con
`import { preflight } from '../../robots/side';`:

- no está bajo `SCAN_ROOTS`, así que `scannedSources()` no lo lee;
- `.gitignore` lo esconde, así que `versionedSources()` tampoco lo nombra y **el
  caso de cobertura (2c/1) no lo puede echar de menos**;
- `resolvesInsideRepository('../../robots/side')` devuelve `true` —resuelve
  dentro del repositorio—, así que el importador **no ofende**.

Medido: `lint exit=0`, `npm test` **772/772**, `tests/polite` **86/86**, y
`git status` no enseña nada. Es la lección de V28 **una capa más afuera**: la
lectura dejó de heredar `.gitignore` **bajo las raíces**, pero la lista de
cobertura —la que hace que quedar fuera sea una decisión declarada— sigue
preguntándoselo a `git`, y `git` no ve lo que `.gitignore` esconde.

### Las evasiones que intenté y MURIERON

Escritas contra el mecanismo nuevo, sobre ficheros reales de `src/`, aplicadas y
revertidas.

| # | Evasión | Resultado |
|---|---|---|
| E9a | La novena, `import` **al principio de la línea** (`src/db/preflight9.ts`, `node:child_process`) | **MUERE**: `architecture` 3 |
| E9b | La novena, **detrás de otra sentencia** — la que dejaba 762/762 | **MUERE**: `architecture` 3 |
| E9c | La novena, **como segundo `import` de la línea** | **MUERE**: `architecture` 3 |
| E12 | Un **symlink** `src/ingest/door.ts` → `impl.txt` con el mismo pago (ni el symlink ni el `.txt` se leen) | **MUERE**: `architecture` **2c** — `git` sí lista el symlink bajo una raíz y la lista leída resulta más estrecha. Buen caso, y hace trabajo real |
| E13 | Una **exclusión declarada** (`SCAN_EXCLUSIONS` con `src/ingest/`) que esconda código versionado | **MUERE**: `architecture` 2b, 2c, 2d, 13, 14. Una exclusión no puede esconder código que `git` sí ve |
| E14 | `src/ingest/robots/side.ts` con `cheerio.fromURL` (F-SPEC-008-V28) | **MUERE DOS VECES**: `architecture` 3 y 13 |
| **E10** | **`.mts` bajo una raíz** | **SOBREVIVE**: 772/772. Ver **F-SPEC-008-V33** |
| **E11** | **`process.getBuiltinModule`** | **SOBREVIVE**: 772/772. Ver **F-SPEC-008-V34** |
| **E15** | **`robots/` en la RAÍZ, fuera de las raíces del escaneo** | **SOBREVIVE**: 772/772. Ver **F-SPEC-008-V35** |

### Las mutaciones de la quinta vuelta, repetidas

Una a una, revertidas, `git status` vacío entre cada una. **Coinciden con lo que
el ledger declara**, con **una precisión sobre R5b**.

| # | Mutación | Resultado medido por el verificador |
|---|---|---|
| R1 | La novena al principio de la línea | **ROJO** `architecture` 3 |
| R2 | La novena tras otra sentencia | **ROJO** `architecture` 3 |
| R3 | La novena como segundo `import` de la línea | **ROJO** `architecture` 3 |
| R4 | `src/ingest/robots/side.ts` con `cheerio.fromURL` | **ROJO DOS VECES**: `architecture` 3 y 13. **Y el control 2d NO la borra**: comprobado que `side.ts` sigue en disco al terminar la suite, y que `hidden-control.ts` sí se limpia |
| R5 | La octava repuesta en **`#captureGranted`** | **ROJO EN LAS DOS MITADES**: `architecture` 3 + `containment` **2, 4, 8, 9** |
| R5b | La misma, en **`capture()`** | **ROJO sólo en `architecture` 3**, confirmado. *Precisión: `assertUserAgent(target.url, this.#userAgent);` aparece **dos veces** en `adapter.ts` —en `capture()` y en el `#attempt` de `tick()`—, así que una mutación hecha con `sed` sobre esa línea engancha las dos y da falsamente el resultado de R5. Con el gancho sólo en `capture()`, R5b es 1 rojo.* |
| R7 | `import` estático de `@/…` **a mitad de línea** por encima de las trampas | **ROJO** `containment` 17. Con el patrón anterior era verde |
| R8 | Un `await import('@/…')` **por encima** de la instalación de la trampa | **ROJO** `containment` 17 (`expected 4929 to be less than 3170`) |
| R9 | `scannedSources()` vuelve a derivarse de `git ls-files --exclude-standard` | **ROJO** `architecture` 2d |
| R10 | Se apaga la comparación contra `compilerModules` | **ROJO** `architecture` 3d |
| R11 | Se apaga el rojo por `unparseable` | **ROJO** `architecture` 3c |
| R12 | El lector deja de enumerar los `export … from` | **ROJO** `architecture` 3, **3b** y 7 |
| R13 | `capabilityOffences` vuelve a mirar texto | **ROJO** `architecture` 9, 10 y **11b** |
| Q-A | `MIN_REQUEST_INTERVAL_MS` 60 000 → 1 000 ms | **ROJO**: 9 casos en 3 ficheros |
| N4 | Segundo parser de `robots.txt` **funcional** en `src/site/robots-txt.ts` | **VERDE — sobrevive, y así tiene que ser**: `tests/polite` + `tests/site` → **184/184**. La pérdida firmada de CA-2.8 (F-SPEC-008-20) **sigue en pie**, y no se ha cerrado por accidente |

### Las cuatro afirmaciones que el encargo mandaba comprobar

1. **`moduleSpecifiers` y `withoutImportStatements` están BORRADOS, no
   envueltos.** `grep -rn "moduleSpecifiers\|withoutImportStatements" src/ tests/`
   → **cero apariciones**. La obligación 1 se cumple, y el caso 3g la comprueba
   con el propio lector.
2. **El caso 9 pasó de dos ofensas a una porque la segunda era un falso positivo,
   y es cierto.** `src/polite/http.ts:32` es
   `fetch(request: HttpRequest): Promise<HttpResponse>;`, **miembro de la
   interfaz `HttpFetcher`**, no una llamada. Las cuatro apariciones de `fetch` en
   el fichero son ese miembro, `fetcher: HttpFetcher`, `fetcher.fetch(…)`, la
   propiedad `fetch:` del objeto literal y `globalThis.fetch(…)`: **ninguna es un
   identificador global desnudo**. Y **`src/polite/http.ts` NO ha cambiado**:
   `git log main..HEAD -- src/polite/http.ts` sólo lista `a95b5ca` y `377091d`,
   de la primera vuelta. R13 lo confirma por el otro lado: con el detector de
   texto repuesto, el caso 9 se pone **rojo** porque vuelven a salir dos.
3. **El control positivo de CA-2.6 ya no borra la mutación del verificador.**
   Repetí R4 con su nombre exacto (`src/ingest/robots/side.ts`) y corrí la suite:
   el caso 2d escribe y borra su propio `hidden-control.ts`, **`side.ts` sigue en
   disco al terminar**, y los casos 3 y 13 se ponen rojos. El caso ya no se pone
   verde por el motivo equivocado.
4. **La octava repuesta muere en las dos mitades sólo si se engancha donde pasa
   el camino conducido**, y así es: en `#captureGranted` mata `architecture` 3 y
   `containment` 2, 4, 8, 9; en `capture()` mata sólo `architecture` 3, porque
   `tick()` no pasa por `capture()`. Es el residuo declarado de CA-2.1 («sólo ve
   lo que se ejecuta») y no un agujero nuevo.

### El refresco del texto (F-SPEC-008-V29), medido

Apagué la secuencia de refresco (`closeFiles` + `fileChanges.changed` + reapertura)
en `currentSnapshot()` y corrí `tests/polite`: **se pone rojo exactamente UN
caso**, el **11** de CA-2.4, que es el único que pide varios controles sobre la
misma ruta sintética (`src/site/probe.ts`, ocho veces). Todos los demás controles
sintéticos usan una ruta propia, así que hoy no se verían afectados —pero **la
alarma entera es ese caso**. El aviso de V29 es exacto y está bien escrito: si
algún día queda un solo control por ruta, deja de haber quien avise. Ratificado
⚠️, no bloquea.

### Lo ya arbitrado, comprobado como registro y no relitigado

- **CA-3 ⚠️** (F-SPEC-008-1) y **CA-8 ⚠️** (F-SPEC-008-2): registradas en la
  matriz con su destino. No se reabren.
- **La pérdida firmada de CA-2.8**: **N4 sigue sobreviviendo**, comprobado a
  propósito (184/184). No hay cambio no declarado.
- **El tamaño de `ALLOWED_PACKAGES` y de sus superficies no es finding.** Catorce
  entradas —`typescript/unstable/ast` y `typescript/unstable/sync` son dos y no
  una, y el motivo está escrito y es correcto: el especificador se juzga tal como
  está escrito—. Lo que sí comprobé: la lista es **literal** (caso 5), es cerrada
  en sus dos ejes y está **ordenada** (caso 4), la concesión es **del grano de la
  capacidad** (4b), el **motivo escrito** está en las cuatro entradas que no se
  explican solas, y **no ha vuelto a esconderse una lista negra en la condición
  de admisión de CA-2.3** —el caso 4c la destaparía—. Leí `capability.ts` entero:
  en `importOffences` no queda un solo nombre cableado. **La lista negra que sí
  queda viva está en CA-2.4**, y es F-SPEC-008-V34.
- **Las 6 h y la licencia de `competition_id: 'robots'`**: firmadas, no se
  reabren, y siguen sujetas por los casos 10-12 de `robots-policy`.

### Findings del verificador — quinta vuelta

- **F-SPEC-008-V33 — ❌ BLOQUEANTE, CA-2.6 y CA-2.3: un `.mts`/`.cts` bajo una
  raíz no está ni en la lista que se lee ni en la de cobertura.**
  `scannedSources()` filtra con `path.endsWith('.ts') || path.endsWith('.tsx')` y
  `versionedSources()` le pasa a `git` el pathspec `'*.ts', '*.tsx'`; **ninguno
  de los dos casa con `.mts` ni con `.cts`**, que son TypeScript que este
  proyecto compila y ejecuta. Medido: `src/ingest/door.mts` con
  `node:child_process` —que **no es entrada de `ALLOWED_PACKAGES`**— enganchado a
  `src/ingest/adapter.ts` deja `lint exit=0`, `npm test` **772/772** y
  `tests/polite` **86/86**; el mismo fichero renombrado a `.ts` es **ROJO** en
  `architecture` 3. **Se commitea con un `git add` normal** (`git status` lo
  enseña, `git check-ignore` no lo tapa), así que a diferencia de V28 **llega a
  producción**. Mandó una petición real, sin User-Agent y sin `robots.txt`,
  contra un servidor local propio.
  **Por qué bloquea y no es residuo**: CA-2.6 se titula «el escaneo cubre todo el
  código del repositorio» y su «NO promete» sólo exime a lo que queda **fuera**
  de las raíces; esto está **dentro**. ADR-016 §6, ya aprobado, obliga a
  declararlo dentro del CA, y no está declarado.
  Destino: **`sdd-implementador`**, en esta misma spec. El arreglo natural es que
  la lista de extensiones que el escaneo lee esté **declarada junto a
  `SCAN_ROOTS`, con su motivo**, como ya lo están las exclusiones, y que la
  cobertura pregunte por lo mismo; enumerar «`.ts` y `.tsx`» dentro de una
  función es la misma clase de lista escondida que este expediente lleva cinco
  vueltas persiguiendo. Ver también §2 de *Lo que necesita decisión humana*.

- **F-SPEC-008-V34 — ❌ BLOQUEANTE, CA-2.4: el conjunto «cerrado por el lenguaje»
  no lo está, y `CAPABILITY_NAMES` es una lista negra de nueve nombres.**
  CA-2.4 afirma que sin un `import` ECMAScript da **exactamente tres** maneras de
  alcanzar una capacidad. Node añade una cuarta que no es de ECMAScript: el
  global del anfitrión **`process`**, y desde Node 22 `process.getBuiltinModule(id)`
  es API **estable** que devuelve cualquier módulo interno. Medido:
  `src/ingest/door11.ts` con
  `const cp = process.getBuiltinModule('node:child_process');` —un `.ts`
  corriente, escaneado y alcanzable desde `ENTRY_POINTS`— deja `lint exit=0`,
  `npm test` **772/772** y `tests/polite` **86/86**, sin ningún `import` que
  CA-2.3 pueda juzgar y sin ningún nombre de `CAPABILITY_NAMES`. Mandó una
  petición real sin User-Agent y sin `robots.txt` contra un servidor local
  propio. CA-2.1 tampoco la ve, porque `node:child_process` no pasa por
  `net.Socket.prototype.connect` — es el residuo cuyo *«único cierre es el
  estático»*, y el estático vuelve a no cerrar.
  **No lo arregla un implementador solo, y por eso va también a decisión**:
  añadir `process` a `CAPABILITY_NAMES` es añadir un nombre a una lista negra,
  que es exactamente lo que ADR-016 §3.1 y la obligación 2 de CA-2.3 prohíben.
  Destino: **`sdd-arquitecto`** sobre CA-2.4 y ADR-016 §5, y **gate humano**.

- **F-SPEC-008-V35 — ⚠️ CA-2.6: fuera de las raíces, la lista de cobertura la
  sigue decidiendo `.gitignore`.**
  La lectura dejó de heredar `.gitignore` **bajo las raíces** (V28 cerrado, R4 lo
  demuestra), pero el caso de cobertura —el que hace que «quedar fuera sea una
  decisión declarada»— sigue saliendo de `versionedSources()`, y `git` no ve lo
  que `.gitignore` esconde. Medido: `robots/side.ts` **en la raíz del
  repositorio**, importado desde `src/ingest/adapter.ts` con una ruta relativa
  que `resolvesInsideRepository` acepta, deja `lint exit=0`, `npm test`
  **772/772** y `tests/polite` **86/86**, y no aparece en `git status`. Es más
  estrecho que V33 —hace falta `git add -f` para commitearlo, igual que en V28—
  pero es la misma lección una capa más afuera. **`.gitignore` no se toca**:
  protege lo que ADR-009 §3 hace irreversible.
  Destino: **`sdd-implementador`**, junto con V33.

- **F-SPEC-008-V29 — ⚠️ ratificado y acotado por medición.** Apagar el refresco
  del texto pone rojo **exactamente un caso** (`architecture` 11). La alarma
  existe y funciona hoy; el aviso de que depende de que ese caso conserve ≥2
  controles por ruta es correcto. Destino: como estaba.
- **F-SPEC-008-V30, V31, V32 — ⚠️ ratificados.** La puerta de conveniencia del
  fallo cerrado (`openedOnDemand`) es real, está nombrada y no afloja nada; el
  cruce contra `sourceFile.imports` está acotado a los literales de módulo, y hoy
  no hay ningún aumento de módulo ni referencia por triple barra en el escaneo;
  el subproceso por fichero de test no mueve ningún gate. Ninguno bloquea.
- **F-SPEC-008-V26 — ⚠️ ratificado.** La trampa muta una propiedad global al
  proceso. Con V33 vivo, además, hay una vía de restauración que CA-2.3 no vería:
  un `.mts` que importe `node:net`.
- **F-SPEC-008-V18, V20, V21, V24, V3, V5, V12 — ⚠️ intactos**, ninguno tocado en
  esta vuelta, todos siguen rutados donde estaban. Ninguno bloquea.
- **F-SPEC-008-1, -2, -3, -4, -6, -8, -16, -17, -18, -19, -20, -21**:
  ratificadas. Ninguna bloquea.
- **F-SPEC-008-7 (el dictamen legal): sigue sin pedir.** Quinta confirmación de
  lo que lo hace tolerable hoy: **en esta rama no se ha corrido nada contra
  `ceroacero.es`**, y en esta verificación lo único que salió a la red fue a
  `127.0.0.1` y a la rama de test de Neon.

### Lo que necesita decisión humana

1. **La pregunta ya no es «¿lee bien el lector?» sino «¿de qué se compone la
   lista que el criterio enumera?», y es la MISMA pregunta de ADR-016 un criterio
   más allá.** CA-2.3 borró su lista negra y quedó bien. **CA-2.4 conserva la
   suya**: `CAPABILITY_NAMES`, nueve nombres, dentro del criterio cuya doctrina
   —ya firmada y aprobada— es *«enumera lo permitido y exige que el resto sea
   vacío»*. La undécima evasión entra por ahí y **no la tapa ningún nombre nuevo
   sin repetir el defecto**. Lo que hay que decidir, y no es mío: si CA-2.4 pasa
   a enumerar **los identificadores globales que un fichero PUEDE usar** —una
   lista blanca, con motivo, como `ALLOWED_PACKAGES`— y todo lo demás es rojo, o
   si se acepta la lista negra y **el residuo se escribe dentro del CA** como
   ADR-016 §6 obliga. Hoy está en la peor de las dos: el CA afirma un conjunto
   «cerrado por el lenguaje» que **está medido abierto**. Destino:
   **`sdd-arquitecto`** sobre CA-2.4 y ADR-016 §5.
2. **La lista de extensiones es el mismo problema en pequeño, y ésta sí la cierra
   un implementador.** `.ts`/`.tsx` está escrito dentro de dos funciones y en un
   pathspec de `git`, no declarado junto a las raíces. Si se declara con su
   motivo —como ya lo están `SCAN_ROOTS` y `SCAN_EXCLUSIONS`— el hueco se cierra
   y el siguiente se ve venir. No hace falta arbitraje.
3. **CA-14 y CA-7 no necesitan nada**, y esta vuelta no los tocó: `git diff
   425fe05 HEAD -- src migrations` está vacío y `npm run test:db` da 144/144.
   `/robot` promete algo que producción ya puede cumplir.
4. **La pregunta legal (F-SPEC-008-7) sigue abierta** y es del humano: el dictamen
   de `sdd-legal-datos` sobre `ceroacero.es` en régimen de ingesta, **antes de
   correr la primera jornada**, no antes de escribir el código.
5. **Nada de lo arbitrado se reabre.** CA-3 ⚠️, CA-8 ⚠️, la pérdida firmada de
   CA-2.8 (N4 sobrevive, 184/184), las 6 h, la licencia de
   `competition_id: 'robots'` y el tamaño de `ALLOWED_PACKAGES`: comprobados como
   registro, no relitigados. Y **no propongo volver a añadir nombres a ninguna
   lista negra**: lo que propongo es borrar la que queda.
...
## Veredicto del verificador — sexta vuelta: RED (2026-09-01, `sdd-verificador`)

**RED, y el finding es uno solo.** No es ninguno de los dos que la enmienda mudó
a SPEC-009, y no es una reapertura de nada arbitrado: es **la misma familia de
F-SPEC-008-V33, una extensión más allá**, y **llega a producción más lejos que
V33**. El mecanismo que la sexta vuelta construyó es **correcto** —lo comprobé
rompiéndolo por cuatro sitios y muerde por los cuatro—; lo que está mal es que
**la declaración se paró en la familia de TypeScript**, y este proyecto compila y
ejecuta JavaScript.

**Lo que sí quedó cerrado, y va primero porque es lo que la enmienda mandaba:**
**F-SPEC-008-V33 está cerrado para `.mts` y `.cts`**, en sus dos mitades, y lo
comprobé escribiendo yo los ficheros y no leyendo los casos. **Las dos listas son
una sola**: no conseguí moverlas por separado. **N4 sobrevive** (187/187). **Nada
se ha tapado por la puerta de atrás.** Los tres gates verdes con
`DATABASE_URL_TEST` disponible: **ningún criterio de CA-14 queda UNMET**.

### Gates — las TRES salidas literales, corridas por el verificador en esta rama

Worktree resuelto con `npm ci` **dentro** del worktree y `.env.local` **copiado**
del checkout principal (fichero de verdad, **nunca un symlink**). `git status`
limpio antes y después de cada mutación, y limpio al commitear. Ningún
`git add -f`. No hizo falta repetir por `ENOTFOUND`.
...
> vitest run
 ...
 RUN  v4.1.11 /Users/albertofojo/src/tremen-dev/marcador.gal/.claude/worktrees/agent-aff1eed92ebbe2263

 Test Files  81 passed (81)
      Tests  775 passed (775)
Type Errors  no errors
   Start at  21:46:22
   Duration  3.92s (transform 1.76s, setup 0ms, import 8.02s, tests 12.01s, environment 4ms, typecheck 147ms)
```
...
> node --env-file-if-exists=.env.local ./node_modules/vitest/vitest.mjs run --config vitest.integration.config.ts tests/db
 ...
 Test Files  8 passed (8)
      Tests  144 passed (144)
   Start at  21:46:30
   Duration  45.90s (transform 75ms, setup 0ms, import 275ms, tests 45.00s, environment 0ms)
```

Y las suites cerradas: `npx vitest run tests/mirror tests/site tests/docs` →
**48 ficheros / 347 casos**. `tests/polite` → **89/89**. Los tres números del
implementador se reproducen exactamente.

### Lo irreversible, primero

- **Ningún HTML real de terceros versionado.** `git diff --name-status main...HEAD`
  no añade **ni un** `.html`/`.htm`. Los `.html` que `git ls-files` lista son de
  `_qa/SPEC-004..006/` —páginas **nuestras**, servidas y capturadas por las
  verificaciones de EPIC-003, ya en `main` desde `c203ba1`— y de `docs/diseno/`
  (EPIC-004). `tests/fixtures/` son tres ficheros `.ts` sintéticos, 11 453 bytes
  en total.
- **Ningún symlink versionado** (`git ls-files -s | awk '$1=="120000"'` → vacío).
- **`.env.local` no versionado**: `git ls-files | grep env` devuelve solo
  `.env.example`.
- **Ningún fichero de mutación superviviente.** Ni en `git ls-files` ni en disco:
  ni `door.*`, ni `preflight*`, ni `side.ts`, ni `outside-roots.*`, ni
  `extension-control.*`, ni `src/ingest/robots/`. Comprobado también bajo
  `robots/`, que `git status` no enseña.
- **`.gitignore` intacto.** Su único diff contra `main` es el `node_modules/` →
  `node_modules` que cerró F-SPEC-008-V22 en `71b2f67`. **La línea 17
  (`**/robots/*`) no se ha tocado**, que es lo que ADR-009 §3 hace irreversible.
- **Aviso de proceso, y lo digo porque casi me lo llevo por delante:** al limpiar
  la reproducción de F-SPEC-008-V35 con `rm -rf robots`, borré
  `robots/.gitkeep`, que **sí está versionado** y que `git status` sí enseña como
  borrado. Restaurado con `git checkout --` en el acto. Quien repita esa mutación
  que borre **el fichero**, no el directorio.

### El alcance del diff, comprobado y no supuesto

`git diff 0dd9930 HEAD` (quinta verificación → HEAD) toca **siete ficheros**, y
de código solo dos: `tests/polite/architecture.test.ts` y
`tests/polite/support/capability.ts`. Los otros cinco son la spec, los dos
ledgers, SPEC-009 y `docs/tablero.md`.

`git diff 9da82ed HEAD -- src migrations .gitignore` está **VACÍO**. **`src/` no
tiene una línea de diff desde la enmienda**, así que la evidencia de los trece CA
✅ se conserva entera y no hay que rehacerla.

### Ataqué F-SPEC-008-V33 por mi cuenta, y está cerrado para lo que nombra

Escribí yo los ficheros, sin mirar los casos 2e/2f/2g, y comparé contra el gemelo
`.ts`. Todos bajo `src/ingest/`, sin enganchar (huérfanos), con
`import { execFileSync } from 'node:child_process'`:

| Fichero | Resultado |
|---|---|
| `src/ingest/vdoor.ts` (referencia) | **ROJO**: `architecture` **3** — `src/ingest/vdoor.ts: node:child_process is not a declared package entry` — y **13** (huérfano) |
| `src/ingest/vdoor.mts` | **ROJO**: `architecture` **3** y **13**, **el mismo caso y el mismo mensaje**, y lo único que cambia es el nombre del fichero |
| `src/ingest/vdoor.cts` | **ROJO**: `architecture` **3** y **13**, ídem |
| `voutside.mts` + `voutside.cts` en la **raíz** | **ROJO**: `architecture` **1** — `expected [ 'voutside.cts', 'voutside.mts' ] to deeply equal []`. La **cobertura los echa de menos**, que es la mitad que antes no existía |

Y el `.cts` bajo `src/ingest/` aparece en `git status` como `?? src/ingest/vdoor.cts`
y en `git ls-files --cached --others --exclude-standard '*.ts' '*.tsx' '*.mts' '*.cts'`:
**la mitad de cobertura también lo ve**. Las dos mitades de V33 están cerradas
para `.mts` y `.cts`.

### Las dos listas son una sola: no pude moverlas por separado

El defecto que V33 nombraba era que `scannedSources()` y `versionedSources()` se
habían quedado cortas **por separado**. Tres mutaciones, cada una intentando
desacoplar **una sola** de las dos:

| # | Mutación | Resultado |
|---|---|---|
| **X1** | `scannedSources()` vuelve a su `endsWith('.ts') \|\| endsWith('.tsx')`, sin tocar `versionedSources()` | **MUERE**: `architecture` **2f** y **2g** |
| **X2** | `versionedSources()` vuelve a escribir a mano `'*.ts', '*.tsx'`, sin tocar `scannedSources()` | **MUERE**: `architecture` **2f** y **2g** |
| **X3** | `extensionPathspec()` **ignora su parámetro** y usa siempre `SCAN_EXTENSIONS` (la parametrización deja de ser real) | **MUERE**: `architecture` **2f** |
| **X4** | Una extensión declarada **sin motivo** (`.ts` con `motive: ''`) | **MUERE**: `architecture` **2e** |
| **X5** | Se **añade** `.js` a la declaración | Se mueven **las dos**: `versionedSources()` empieza a listar `docs/diseno/_logic.js` y `architecture` **1** se pone rojo. La declaración manda de verdad, en los dos sentidos |

**No hay forma de mover una sin la otra.** X5 es además la prueba de que la
declaración es el **único** sitio donde tocar: es lo que hace que el finding de
abajo se arregle con un diff y no con un mecanismo nuevo.

### EL FINDING: hay una QUINTA forma, y es JavaScript

La pregunta del encargo era si queda alguna extensión que el proyecto **compile o
ejecute** y que la declaración no cubra. **La hay, y son cuatro**: `.js`, `.jsx`,
`.mjs` y `.cjs`. No es hipotética por dos motivos independientes:

1. **Este repositorio ya versiona y ejecuta `.mjs`**: `docs/diseno/build.mjs`
   monta los artboards de EPIC-004, y `docs/diseno/_logic.js` es el fichero que
   obligó a `ignorePatterns` en F-SPEC-008-5. `package.json` declara
   `"type": "module"`, así que un `.js` bajo `src/` es ESM y Node lo ejecuta.
2. **Next.js compila y sirve `.js` en `src/app/`**, y eso **lo medí, no lo
   supuse**.

**La medición, entera.** Escribí `src/app/(gl)/vprobe/route.js`:

```js
import { execFileSync } from 'node:child_process';

export function GET() {
  const out = execFileSync('echo', ['vprobe'], { encoding: 'utf8' });
  return new Response(out);
}
```

- `npx next build` → **compila** y lo pone en la tabla de rutas de producción:
  `└ ƒ /vprobe` (*Dynamic — server-rendered on demand*).
- `npx next start` + `curl http://127.0.0.1:3987/vprobe` → **`vprobe`**. La ruta
  **se sirve y el `node:child_process` se ejecuta de verdad**.
- `npm run lint` → **exit=0**. `npm test` → **775/775**. `tests/polite` →
  **89/89**. `npm run test:db` → **144/144**.
- `git status` → `?? src/app/(gl)/vprobe/route.js`. **Se commitea con un
  `git add` normal**, sin `git add -f`.
- `git ls-files --cached --others --exclude-standard '*.ts' '*.tsx' '*.mts' '*.cts'`
  → **no lo lista**. Ni `scannedSources()` lo lee ni `versionedSources()` lo echa
  de menos. **Las dos listas, que ahora son una, son ciegas a la vez.**

Y la variante barata, medida aparte: `src/ingest/vdoor.mjs` con el mismo
`node:child_process` deja **`lint exit=0`, `npm test` 775/775 y `tests/polite`
89/89**, y `node -e "import('./src/ingest/vdoor.mjs')…"` lo **ejecuta**.

**Por qué es incumplimiento de CA-2.6 y no una lista que se quedó corta sin más.**
CA-2.6 cierra con una promesa literal: *«lo que el criterio garantiza es que
**quedar fuera sea una decisión declarada en las raíces**, no un efecto colateral
de otra regla»*. Hoy JavaScript queda fuera y **no hay ninguna decisión declarada
que lo diga**: no hay entrada en `SCAN_EXTENSIONS`, no hay motivo escrito, y no
hay entrada en `SCAN_EXCLUSIONS`. Es exactamente un efecto colateral —la
declaración se escribió para cerrar `.mts` y se detuvo en la familia de
TypeScript—. Y el motivo escrito de la entrada `.cts` dice, con estas palabras,
*«declarada ANTES de que nadie escriba uno. Una lista que solo crece después de la
medición es una lista que llega tarde»*: `.mjs` **ya está escrito y versionado en
este repositorio**, así que por su propio principio llegó tarde.

**Es peor que F-SPEC-008-V33 en la dimensión que decidió su destino.** El
`door.mts` de V33 era un fichero huérfano que nadie importaba y que solo llegaba
a producción en el sentido de que `git add` lo commitea. Éste **es un endpoint
HTTP vivo en la tabla de rutas de la aplicación construida**, alcanzable desde
fuera. La regla con la que Alberto separó lo que se arregla aquí de lo que se
muda —*«llega a producción con un `git add` normal»*— lo señala con más fuerza que
al caso que la motivó.

**Y es de SPEC-008, no de SPEC-009.** Lo que mandó a F-SPEC-008-V34 y
F-SPEC-008-V35 a SPEC-009 es que cerrarlos **exige un cambio de criterio con
firma humana**: V34 pide sustituir una lista negra por una lista blanca de
identificadores globales —la pregunta abierta de SPEC-009, notas §16— y V35 toca
la frontera de `git`/`.gitignore`. Cerrar éste, en cambio, es lo que el propio
CA-2.6.5 llama, con esas palabras, *«un diff con motivo, como una entrada de
`ALLOWED_PACKAGES`, y **nunca un arbitraje**»*: cuatro entradas en
`SCAN_EXTENSIONS` y, porque X5 lo destapa, una exclusión declarada con su motivo
para `docs/diseno/` (o su entrada como raíz). **Ninguna firma humana, ningún
mecanismo nuevo, y el mecanismo que hace falta ya está construido y ya muerde.**

**La lectura contraria, dicha para que el gate pueda arbitrarla si no está de
acuerdo:** CA-2.6.5 dice *«Hoy son las de TypeScript que este proyecto compila y
ejecuta»*, y se puede leer como que el criterio se acota **a propósito** a la
familia de TypeScript, lo que dejaría JavaScript fuera de su alcance y mandaría
esto a SPEC-009 con V34, V35 y V36. **No lo leo así** —esa frase describe la
lista de hoy, no declara una exclusión, y ninguna de las dos cosas que CA-2.6
exige de una exclusión (entrada y motivo) existe para JavaScript—, pero la
diferencia entre las dos lecturas es una decisión de alcance y el sitio de
zanjarla es el gate humano, no este informe.

### El residuo escrito coincide con el residuo real, y no lo encoge

Reproduje los dos que la enmienda declara abiertos, para comprobar que el texto
no promete de más **ni de menos**:

- **F-SPEC-008-V34 — sigue abierto, exactamente como CA-2.4 lo escribe.**
  `src/ingest/vprobe34.ts` con
  `const cp = process.getBuiltinModule('node:child_process');`, **enganchado** a
  `src/ingest/adapter.ts` con un `export { … } from './vprobe34'` para que no sea
  huérfano y sea alcanzable desde `ENTRY_POINTS`: **`lint exit=0`, `npm test`
  775/775, `tests/polite` 89/89**. Sin enganchar solo muerde `architecture` 13,
  por huérfano y no por la capacidad. El residuo de CA-2.4 es **cierto**.
- **F-SPEC-008-V35 — sigue abierto, exactamente como CA-2.6 lo escribe.**
  `robots/side.ts` en la raíz del repositorio: `git status` **no enseña nada**,
  `versionedSources()` **no lo lista**, y `tests/polite` sigue en **89/89**. El
  residuo de CA-2.6 es **cierto**.
- **F-SPEC-008-V36** (la cuarta lista de extensiones, en `resolveModule`) se
  confirma **fallando cerrado**: mis `.mts` y `.cts` ponen rojo `architecture`
  **13** por huérfanos, que es el rojo falso que la salvedad describe. **No es un
  verde falso** y no cambia de destino.

**Lo que el residuo escrito NO recoge es el finding de arriba**, y ésa es la
segunda mitad del RED: *«Lo que esta parte NO promete»* de CA-2.6 nombra el hueco
de cobertura fuera de las raíces y no nombra JavaScript, así que **el residuo
escrito hoy es más pequeño que el residuo real**.

### Nada se ha tapado por la puerta de atrás

- **`CAPABILITY_NAMES` intacto.** Sigue siendo la lista de **nueve** nombres
  —`globalThis`, `fetch`, `XMLHttpRequest`, `WebSocket`, `EventSource`,
  `navigator`, `eval`, `Function`, `require`—. `git diff 0dd9930 HEAD` sobre
  `capability.ts` no toca ni una línea suya.
- **`process` no está en ninguna lista.** `grep` sobre `capability.ts` → cero
  apariciones como entrada. ADR-016 §3.5 respetado.
- **`versionedSources()` no se ha ensanchado por F-SPEC-008-V35**: sigue
  preguntándole a `git`, y por eso V35 se reproduce igual.
- **`.gitignore` sin tocar**, **`src/` y `migrations/` sin tocar** desde la
  enmienda (diff vacío).
- **`ALLOWED_PACKAGES` sin tocar** en esta vuelta.

### N4 sigue sobreviviendo, y es lo que tiene que pasar

Reescribí el segundo parser de `robots.txt` **funcional** dentro de
`src/site/robots-txt.ts` —`secondRobotsParser` y `secondIsAllowed`, con desempate
por longitud—: `npx vitest run tests/polite tests/site` → **18 ficheros / 187
casos, todos verdes**. Coincide con el número del implementador. **La pérdida
firmada de CA-2.8 (F-SPEC-008-20) sigue en pie y esta vuelta no la ha cerrado por
accidente.** Revertido.

### CA-7 y CA-14 siguen ciertos, comprobados rompiendo

- **CA-7.** `MIN_REQUEST_INTERVAL_MS` de `60_000` a `1_000` → **9 casos rojos en 3
  ficheros**: `adapter` 10, 11, 12, 13; `polite/rate-limit` 2 y 7; y
  `mirror/capture/rate-limit` 1, 2, 4. El número es load-bearing **en las dos
  mitades**, la de esta spec y la del instrumento de SPEC-002. Mismo resultado que
  la cuarta y la quinta verificación.
- **CA-14.7 — se falla cerrado.** `capture()` con
  `.catch(() => true)` sobre `takeTurn` → **ROJO** en `adapter` **19**
  («`capture()`: cero peticiones, cero bytes archivados y el error sin envolver»).
- **CA-14 durable.** El `where` de `src/db/rate-limit.ts` con `${now}` en vez de
  `${limit}` —conceder siempre— → **8 casos rojos** en `npm run test:db`: la
  batería de contrato 2, 5, 6, 7, más **CA-14.2** (3), **CA-14.4** (5 y 6) y
  **CA-14.6** (7). El caso 5 es la cláusula de no acumular que `/robot` publica en
  galego y en castellano.

### Las suites de SPEC-002 a SPEC-007 siguen enteras

Recuento fichero a fichero con `git grep -c '  test('`, `main` contra `HEAD`,
sobre `tests/mirror tests/site tests/docs`: **44 ficheros en cada referencia** y
**una sola línea de diferencia**, `tests/mirror/user-agent.test.ts` **15 → 16**,
que **sube**. **Ningún caso borrado.** Y sobre las suites de SPEC-001
(`tests/model tests/raw tests/db tests/types`), la única diferencia es el fichero
**nuevo** `tests/db/rate-limit.test.ts:7`. Corridas: **48 ficheros / 347 casos**
verdes. F-SPEC-008-1 sigue **arbitrada y no revisitada**.

### Lo ya arbitrado, comprobado como registro y no relitigado

Las 6 h de ADR-014 §3.2, la licencia de `competition_id: 'robots'`,
F-SPEC-008-1 (CA-3 ⚠️), F-SPEC-008-2 (CA-8 ⚠️), el tamaño de `ALLOWED_PACKAGES` y
**la pérdida firmada de CA-2.8**: comprobados como registro, **no reabiertos**.
Y **CA-2 tiene ⚠️ como estado máximo alcanzable** por la enmienda del 2026-09-01:
el verde de CA-2.6 no lo sube a ✅, y esta verificación no lo intenta.

### Findings del verificador — sexta vuelta

- **F-SPEC-008-V37 — ❌ BLOQUEANTE. El escaneo no lee JavaScript, y este proyecto
  compila y ejecuta JavaScript. Destino: SPEC-008, en CA-2.6.**
  `SCAN_EXTENSIONS` declara `.ts`, `.tsx`, `.mts` y `.cts`, y **no declara nada
  sobre `.js`, `.jsx`, `.mjs` ni `.cjs`** —ni como entrada ni como exclusión con
  su motivo—, así que quedan fuera **por efecto colateral y no por decisión
  declarada**, que es justo lo que CA-2.6 garantiza que no pasa.
  **Medido**: `src/app/(gl)/vprobe/route.js` con
  `import { execFileSync } from 'node:child_process'` lo **compila `next build`**
  (`└ ƒ /vprobe` en la tabla de rutas de producción), lo **sirve `next start`**
  (`curl /vprobe` → `vprobe`), deja **`lint exit=0`, `npm test` 775/775,
  `tests/polite` 89/89, `test:db` 144/144**, y **se commitea con un `git add`
  normal**. Ni `scannedSources()` lo lee ni `versionedSources()` lo echa de menos.
  Variante barata: `src/ingest/vdoor.mjs`, mismos números.
  **Cómo se arregla** —y es un diff, no un arbitraje, por el propio CA-2.6.5—:
  (1) declarar `.js`, `.jsx`, `.mjs` y `.cjs` en `SCAN_EXTENSIONS` con su motivo;
  (2) el control positivo de CA-2.6.7 escrito también para un `.js` bajo
  `src/app/`, rojo **por el mismo caso** que su gemelo `.ts`; y (3) lo que X5
  destapa: con `.js` declarado, `versionedSources()` empieza a listar
  `docs/diseno/_logic.js` y `docs/diseno/build.mjs`, así que hay que **declarar
  `docs/diseno/` como exclusión con su motivo** (EPIC-004, congelada; `_logic.js`
  no es un módulo completo y `.oxlintrc.json` ya lo excluye por eso) o traerlo
  bajo una raíz. Nada de esto toca `src/`, `migrations/`, `.gitignore` ni
  `CAPABILITY_NAMES`.
  **Lo que NO hay que hacer**: ensanchar `resolveModule` (eso es F-SPEC-008-V36 y
  es de SPEC-009), ni tocar `versionedSources()` por F-SPEC-008-V35, ni añadir
  `process` a ninguna lista.

- **Lo que NO es finding mío y pertenece a SPEC-009**, dicho aquí porque rutar
  bien importa más que encontrar otra evasión: **F-SPEC-008-V34**
  (`process.getBuiltinModule`, `CAPABILITY_NAMES` como lista negra),
  **F-SPEC-008-V35** (la cobertura fuera de las raíces) y **F-SPEC-008-V36** (la
  cuarta lista de extensiones en `resolveModule`). Los tres los **reproduje** y
  los tres siguen abiertos **exactamente como su residuo los describe**. No son
  findings que reabrir: son alcance de otra spec, y su residuo está bien escrito.

### Lo que necesita decisión humana

1. **Si F-SPEC-008-V37 es de esta spec o de SPEC-009.** Mi juicio, con la medida
   delante, es que es de ésta: llega a producción como un endpoint servido, su
   arreglo es un diff con motivo que el propio CA declara «nunca un arbitraje», y
   es la misma familia del finding que esta vuelta existía para cerrar. La lectura
   contraria —que CA-2.6.5 se acota a propósito a TypeScript— está escrita arriba
   sin suavizar. **Si el gate prefiere mudarlo, SPEC-009 está en `borrador` y es
   el sitio; pero entonces conviene decirlo dentro de CA-2.6, porque hoy el
   residuo escrito no lo nombra.**
2. **F-SPEC-008-7 sigue sin pedir**: el dictamen de `sdd-legal-datos` sobre
   `ceroacero.es` en régimen de ingesta. Sigue siendo precondición de correr la
   primera jornada, no de escribir código, y sigue siendo del humano.

### ¿Puede cerrarse esta spec?

**Todavía no, y le falta poco.** Trece CA están ✅ con su evidencia reproducida,
cuatro están ⚠️ con su residuo arbitrado y escrito, y **CA-2 se queda en ❌ por un
solo finding cuyo arreglo cabe en una vuelta tan corta como la sexta** —cuatro
entradas declaradas, una exclusión con su motivo, y un control positivo—. El
mecanismo que hace falta **ya está construido, ya es una sola declaración y ya
muerde**: lo único que le falta es que la declaración diga la verdad sobre qué
compila y ejecuta este proyecto. **Cerrada esa línea, y salvo que aparezca una
forma nueva, esta spec puede cerrarse en la séptima con CA-2 en ⚠️**, que es su
estado máximo alcanzable por decisión de la enmienda.

## Séptima vuelta — la declaración dice la verdad sobre qué compila y ejecuta este proyecto (2026-09-01, `sdd-implementador`)

**Más corta todavía que la sexta, y por el mismo motivo: el mecanismo ya estaba
construido y lo que fallaba era lo que declaraba.** El encargo es **un solo
finding**, **F-SPEC-008-V37**, y su arreglo es lo que el propio **CA-2.6.5** llama
*«un diff con motivo… **nunca un arbitraje**»*: cuatro entradas nuevas en
`SCAN_EXTENSIONS`, una exclusión nueva con su motivo, y los controles positivos
que lo sostienen.

Los trece CA ✅ y los ⚠️ arbitrados **no se tocaron**. `src/`, `migrations/`,
`.gitignore`, `.oxlintrc.json`, `docs/roadmap.md`, `CLAUDE.md`, ADR-014 y
SPEC-009 salen de esta vuelta con **cero líneas de diff**. Lo único que cambia
son **dos ficheros bajo `tests/`** y este ledger.

### 1. Qué era el agujero, y por qué es peor que el de la sexta vuelta

`SCAN_EXTENSIONS` se paró en la familia de TypeScript —`.ts`, `.tsx`, `.mts`,
`.cts`—, así que **`.js`, `.jsx`, `.mjs` y `.cjs` quedaban fuera por efecto
colateral**: ni entrada, ni exclusión con motivo. Es exactamente lo que CA-2.6
garantiza que no pasa —*«lo que el criterio garantiza es que quedar fuera sea una
decisión declarada en las raíces, no un efecto colateral de otra regla»*—, y esa
frase no distingue entre quedar fuera por una raíz y quedar fuera por una
extensión.

Y la diferencia con F-SPEC-008-V33 no es de grado: **aquél era un huérfano; éste
es un endpoint HTTP vivo.** `src/app/(gl)/vprobe/route.js` con
`import { execFileSync } from 'node:child_process'` lo **compila `next build`**
(`ƒ /vprobe` en la tabla de rutas de producción), lo **sirve `next start`**, se
**commitea con un `git add` normal**, y ninguna de las dos listas lo veía.
`node:child_process` **no es entrada de `ALLOWED_PACKAGES`**, y es precisamente el
residuo cuyo «único cierre es el estático» según CA-2.1.

### 2. Con qué se cierra, y son dos líneas de declaración y ninguna de mecanismo

**Nada del mecanismo de la sexta vuelta cambia.** Sigue habiendo **una sola
declaración** de qué es un fichero de código, y `scannedSources()` y
`versionedSources()` la siguen preguntando las dos. Lo que cambia es **lo que
declara**.

1. **Las cuatro extensiones de JavaScript, cada una con su motivo escrito**,
   junto a `SCAN_ROOTS` y `SCAN_EXCLUSIONS`, y **no dentro de una función ni
   dentro de un pathspec**:
   - **`.js`** — «Next compila y sirve una ruta escrita en JavaScript a secas
     —medido, `ƒ /vprobe` en la tabla de rutas de producción— y Node lo ejecuta.
     Es código que este repositorio despliega (F-SPEC-008-V37)».
   - **`.jsx`** — lo mismo con JSX, **declarada antes de que exista una**, como
     `.cts`: nada aquí prohíbe una ruta o un componente en `.jsx`, y una lista
     que espera a la medición llega tarde.
   - **`.mjs`** — el módulo ESM explícito de JavaScript, que es la variante
     barata de la misma evasión (`src/ingest/vdoor.mjs`, los mismos números).
   - **`.cjs`** — el gemelo CommonJS de `.mjs`, y aquí importa un punto más:
     **CA-2.4 cierra `require`, y `require` es lo que escribe un `.cjs`**.
2. **`docs/diseno/` como exclusión declarada, con su motivo**, que es lo que
   destapa la mutación **X5** del verificador: en cuanto `.js` y `.mjs` son
   código, `git` empieza a listar `docs/diseno/_logic.js` y `docs/diseno/build.mjs`
   y la cobertura del caso 1 se pone roja. **No son código de la aplicación**:
   son las fuentes del sistema de diseño de **EPIC-004** —congelada—, nada bajo
   `src/` las importa, y **`_logic.js` ni siquiera es JavaScript autónomo**: es
   el bloque que `build.mjs` inyecta en los artboards. **Es la misma frontera que
   el gate de calidad ya trazó**: `_logic.js` rompió `oxlint` el 2026-09-01 y
   `.oxlintrc.json` ignora `docs/diseno/` desde entonces (**commit `5b632df`,
   «Gate de calidad: oxlint ignora docs/diseno, que no es codigo»**). Se cita ahí
   como precedente, dentro del motivo.
3. **La exclusión gobierna las dos listas, no una.** `excluded()`,
   `scannedSources()` y `versionedSources()` pasan a aceptar la declaración de
   exclusiones como parámetro —igual que ya aceptaban la de extensiones—, y
   `versionedSources()` la aplica. Una exclusión que solo silenciara una de las
   dos sería la forma de F-SPEC-008-V33 otra vez.

**No se ha ensanchado ninguna lista negra y no se ha añadido ningún mecanismo
nuevo.**

### 3. Los dos casos nuevos, y qué mecanismo vigila cada uno

`tests/polite/architecture.test.ts`, en el `describe` de CA-2.6:

- **2h — `docs/diseno/` es una exclusión DECLARADA, y sin ella la cobertura la
  echaría de menos.** No comprueba solo que el nombre esté en la lista: comprueba
  que **sostiene algo**, corriendo `versionedSources()` con la declaración **sin**
  esa entrada y exigiendo que entonces `docs/diseno/_logic.js` y
  `docs/diseno/build.mjs` aparezcan como no cubiertos. Se pregunta por **esos dos
  ficheros y no por el conjunto entero**, por lo mismo que 2g: el conjunto lo
  juzga el caso 1, y este caso no tiene por qué ponerse rojo por la mutación de
  otro.
- **2i — control positivo, reproducción exacta de F-SPEC-008-V37.** Escribe
  ficheros **reales** —un `src/app/(gl)/js-control/route.js` y un
  `src/ingest/extension-control-js.mjs`, los dos con `node:child_process`—,
  comprueba que **el escaneo los lee** (`scannedSources()`), que **el compilador
  los parsea** (`unparseable === false`) y que **son rojos con el mismo mensaje
  que su gemelo `.ts`** salvo el nombre del fichero; y la mitad de **cobertura**:
  `git` los lista, y otro escrito **fuera** de las raíces aparece en la lista que
  el caso 1 juzga. Los tres ficheros se borran en `finally`.
  **Nombre propio**, como 2d y 2g: el verificador repite la mutación con `vprobe`
  y `vdoor`, y un control que pisara esos nombres borraría su mutación. **El
  directorio se retira con `rmdirSync` no recursivo** y solo si no existía antes:
  un `rm -rf` en un control es lo que se llevó por delante un `.gitkeep`
  versionado en una vuelta anterior.

Y dos casos existentes mueven su expectativa, **sin aflojarse**: **2b** (las
exclusiones son `node_modules/` **y** `docs/diseno/`, las dos con motivo) y
**2e** (la declaración son ocho sufijos, todos con motivo, todos empezando por
`.` y ninguno con `*`).

### 4. «Comprobado que no rompe nada», y está medido, no supuesto

El encargo pedía comprobarlo y decirlo con evidencia si movía algo. **No mueve
nada.** Medido comparando las dos listas con la declaración recortada a la
familia de TypeScript y con la declaración entera:

```
{"scannedBefore":89,"scannedAfter":89,"scannedNew":[],
 "versionedBefore":89,"versionedAfter":89,"versionedNew":[],"versionedLost":[]}
```

**Ni un fichero entra, ni un fichero sale.** Coincide con lo que dice el encargo:
hoy no hay **ni un** `.js`, `.jsx`, `.mjs` o `.cjs` versionado bajo `src/`
(`git ls-files '*.js' '*.jsx' '*.mjs' '*.cjs'` → **solo** `docs/diseno/_logic.js`
y `docs/diseno/build.mjs`). Y `versionedLost: []` dice que la exclusión nueva
**no quita de la cobertura nada que antes estuviera**.

### 5. Gates — las TRES salidas literales
...
> vitest run
 ...
 RUN  v4.1.11 /Users/albertofojo/src/tremen-dev/marcador.gal/.claude/worktrees/agent-ab8e69295d9814c26

 Test Files  81 passed (81)
      Tests  777 passed (777)
Type Errors  no errors
   Start at  22:04:45
   Duration  4.07s (transform 2.04s, setup 0ms, import 8.37s, tests 11.17s, environment 4ms, typecheck 122ms)
```
...
> node --env-file-if-exists=.env.local ./node_modules/vitest/vitest.mjs run --config vitest.integration.config.ts tests/db
 ...
 Test Files  8 passed (8)
      Tests  144 passed (144)
   Start at  22:04:53
   Duration  47.72s (transform 76ms, setup 0ms, import 273ms, tests 46.86s, environment 0ms)
```

Y las suites de las specs cerradas, sin mover una coma:
`npx vitest run tests/mirror tests/site tests/docs` → **48 ficheros / 347 casos**.

**775 → 777**: dos casos nuevos, **ninguno borrado**. `tests/polite`: **89 → 91**.

### 6. Mutaciones — nueve, aplicadas una a una y revertidas

`git status` limpio entre cada una.

| # | Mutación | Resultado |
|---|---|---|
| **T1** | **F-SPEC-008-V37 tal cual**: `src/app/(gl)/vprobe/route.js` con `import { execFileSync } from 'node:child_process'`. **Antes dejaba `lint exit=0`, `npm test` 775/775, `tests/polite` 89/89 y `test:db` 144/144, y lo servía `next start`** | **MUERE**: `architecture` **3** con `src/app/(gl)/vprobe/route.js: node:child_process is not a declared package entry` —el mismo mensaje que su gemelo `.ts`, salvo el nombre— y además `architecture` **17** (`ENTRY_POINTS` no la nombra: una ruta que ahora se lee es una ruta que hay que declarar) |
| **T2** | La variante barata: `src/ingest/vdoor.mjs`, sin enganchar | **MUERE**: `architecture` **3** (`src/ingest/vdoor.mjs: node:child_process is not a declared package entry`) y **13** (huérfano) |
| **T3** | Los dos gemelos declarados **antes de que existan**: `src/ingest/vdoor.cjs` con `require('node:child_process')` y `src/site/vwidget.jsx` con el `import` | **MUEREN, y el `.cjs` por CUATRO caminos**: `architecture` **3** (`the compiler names node:child_process and the reader did not enumerate it` — un `require()` **falla cerrado**), **3b**, **8** (CA-2.4, `src/ingest/vdoor.cjs: require`) y **13**. El `.jsx`: **3** y **13** |
| **T4** | Se borra `.js` de `SCAN_EXTENSIONS` | **MUERE**: `architecture` **2e**, **2h** y **2i** |
| **T5** | Se borra `docs/diseno/` de `SCAN_EXCLUSIONS` | **MUERE**: `architecture` **1** (`expected [ 'docs/diseno/_logic.js', 'docs/diseno/build.mjs' ] to deeply equal []`), **2b** y **2h** |
| **T6** | `versionedSources()` **ignora su parámetro** de exclusiones y usa siempre `SCAN_EXCLUSIONS` | **MUERE**: `architecture` **2h** (la exclusión deja de poder demostrarse load-bearing) |
| **T6b** | `versionedSources()` **deja de aplicar** las exclusiones | **MUERE**: `architecture` **1** y **2h** |
| **T7** | Un `.js` **fuera** de las raíces, en la raíz del repositorio (`vroot.js`) | **MUERE**: `architecture` **1** (`expected [ 'vroot.js' ] to deeply equal []`) |
| **T8** | Una exclusión sin motivo (`docs/diseno/` con `motive: ''`) y, aparte, una extensión sin motivo (`.js` con `motive: ''`) | **MUEREN**: `architecture` **2b** + **2h** la primera, **2e** la segunda |
| **T9** | *(el coste declarado, no un defecto)*: `docs/diseno/vside.js` con `node:child_process` | **VERDE — sobrevive, y es lo que la exclusión declara.** Ver salvedad F-SPEC-008-V38 |
| **N4** | Segundo parser de `robots.txt` **funcional** dentro de `src/site/robots-txt.ts` | **VERDE — sobrevive, y así tiene que ser.** `tests/polite` + `tests/site` → **189/189**. La pérdida firmada de CA-2.8 (**F-SPEC-008-20**) **sigue en pie**: esta vuelta no la cierra por accidente |

### 7. Lo que esta vuelta NO tocó, comprobado y no supuesto

- **`CAPABILITY_NAMES` no se ha tocado y no se le ha añadido `process`.** Sigue
  siendo la lista de nueve nombres que CA-2.4 declara como residuo, y
  **F-SPEC-008-V34 sigue abierto a propósito**: es de SPEC-009, y taparlo con un
  nombre nuevo sería el movimiento que ADR-016 §3.5 prohíbe.
- **`versionedSources()` no se ha ensanchado por F-SPEC-008-V35.** Lo único que
  se le añadió es que aplique **las exclusiones declaradas**; **fuera de las
  raíces la cobertura la sigue decidiendo `.gitignore`**, exactamente como CA-2.6
  lo deja escrito. **Es de SPEC-009.**
- **`resolveModule()` no se ha ensanchado (F-SPEC-008-V36).** Sigue con su cuarta
  lista de extensiones. Ver la salvedad de abajo: el ensanche de esta vuelta
  **agranda ese rojo falso**, y **no se arregla aquí**.
- **`.gitignore` y `.oxlintrc.json` intactos** (el primero protege lo que ADR-009
  §3 hace irreversible; el segundo se **cita** como precedente, no se toca).
- **`src/` no tiene una línea de diff.** `src/site/robots-txt.ts` y
  `src/mirror/analysis/referenceless/report.ts` (CA-3) intactos; CA-7, CA-14,
  ADR-014, `docs/roadmap.md`, `CLAUDE.md` y **SPEC-009** intactos.
- **El dictamen legal (F-SPEC-008-7) sigue sin pedir**: es del humano.
- **Ninguna petición a un tercero** en toda la vuelta.

### 8. Salvedades nuevas

- **F-SPEC-008-V38 — ⚠️ lo que la exclusión de `docs/diseno/` cuesta, dicho con
  la medida delante. Es el precio declarado, no un agujero, y NO se arregla
  aquí.** Con `docs/diseno/` excluida, **un `docs/diseno/vside.js` con
  `node:child_process` deja las tres salidas verdes** (medido, T9): nada de ahí
  se lee ni se echa de menos. Eso es exactamente lo que CA-2.6 permite —*«un
  fichero fuera de todas las raíces sigue sin leerse, y eso es deliberado»*— **a
  condición de que sea una decisión declarada, que es lo que ahora es**. Lo que
  lo hace tolerable y no una puerta: `docs/diseno/` **no se despliega** (no cuelga
  de `src/app/` ni de ninguna raíz de Next), **nada de `src/` lo importa** —y si
  algún día lo importara, el especificador sería una ruta relativa que
  `resolvesInsideRepository` acepta y el fichero quedaría igualmente sin leer, que
  es la forma de F-SPEC-008-V35—, y **es material de EPIC-004, congelada**. **Qué
  lo despierta**: el día que algo bajo una raíz importe algo de `docs/diseno/`, o
  el día que EPIC-004 se descongele y sus fuentes pasen a ser código que alguien
  despliega. **Destino: SPEC-009**, que es la spec de dónde vive la declaración de
  qué es código, junto a V34, V35 y V36.
- **F-SPEC-008-V36 se agranda con este diff, y va dicho porque el encargo lo
  pedía.** `resolveModule()` solo resuelve `.ts`/`.tsx`, así que **un `.js`,
  `.jsx`, `.mjs` o `.cjs` legítimo bajo `src/ingest/`, `src/polite/` o
  `src/site/` quedaría huérfano y pondría rojo `architecture` 13** aunque nadie
  hiciera nada malo — medido en T2 y T3. **El modo de fallo sigue siendo un rojo
  falso y nunca un verde falso**, así que no se escapa nada por ahí; pero la
  superficie del rojo falso pasa de dos extensiones a seis. **No se arregla
  aquí**, porque es F-SPEC-008-V36 y es de SPEC-009, y porque tocar el resolvedor
  mueve el conjunto `reachable` del que dependen los casos 13, 15, 16 y 17.
  **Qué lo despierta** (sin cambio): el día que entre en el repositorio un
  `.mts`/`.cts`/`.js`/`.mjs` legítimo.
- **Nada más.** No se abrió ningún finding de `src/`, porque esta vuelta no lo
  toca.

### 9. Cómo retomar (handoff)

- **Rama**: `ft/SPEC-008-adaptador-ceroacero`. **Sin push y sin PR.**
- **Commit de esta vuelta**: `test(SPEC-008): el escaneo lee JavaScript, y
  docs/diseno queda fuera por decision declarada` — **dos ficheros bajo `tests/`**
  más el ledger.
- **Worktree**: `npm ci` **dentro** del worktree y copiar `.env.local` del
  checkout principal (para `DATABASE_URL_TEST`). **Nunca un symlink**, nunca
  `git add -f`. Si `test:db` da `ENOTFOUND` contra Neon, es DNS: repetir.
- **Dónde mirar primero**: `tests/polite/support/capability.ts` —`SCAN_EXTENSIONS`
  (ocho sufijos con motivo) y `SCAN_EXCLUSIONS` (`node_modules/` y
  `docs/diseno/`)— y los casos **2b**, **2e**, **2h** y **2i** de
  `tests/polite/architecture.test.ts`.
- **Para repetir la mutación del finding**: `src/app/(gl)/vprobe/route.js` con
  `node:child_process`, y `src/ingest/vdoor.mjs` como variante barata. El control
  2i usa **otros nombres** (`js-control/route.js`, `extension-control-js.mjs`),
  así que **no los borra** y conviven. **Al limpiar, borra el fichero y luego el
  directorio con `rmdir`, nunca `rm -rf`**: un `rm -rf robots` se llevó por
  delante un `robots/.gitkeep` versionado en una vuelta anterior. Ojo también con
  2d y `src/ingest/robots/`: `git status` no muestra nada bajo un directorio
  `robots/`.
- **Lo que sigue abierto y no es del implementador**: **F-SPEC-008-V34**,
  **F-SPEC-008-V35**, **F-SPEC-008-V36** y **F-SPEC-008-V38** (los cuatro a
  SPEC-009), y **F-SPEC-008-7**, el dictamen de `sdd-legal-datos`.
- **Lo que hay que juzgar en la séptima verificación**: **CA-2 tiene ⚠️ como
  estado máximo alcanzable** por decisión de la enmienda del 2026-09-01. Verde en
  CA-2.6 no es ✅ de CA-2.

## Veredicto del verificador — séptima vuelta: GREEN (2026-09-01, `sdd-verificador`)

**GREEN.** **F-SPEC-008-V37 está cerrado**, y lo comprobé escribiendo yo los
ficheros y reproduciendo la ruta viva de punta a punta, no leyendo los casos. Los
tres gates verdes con `DATABASE_URL_TEST` disponible: **ningún criterio de CA-14
queda UNMET**. **17 filas de la matriz ✅, cinco ⚠️, ninguna ❌.**

**CA-2 se cierra en ⚠️, que es su techo firmado** por la enmienda del 2026-09-01
(«CA-2 se queda ⚠️ con su residuo escrito, y la frontera de capacidad sale a
SPEC-009»). Verde en CA-2.6 **no** lo sube a ✅, y esta verificación no lo intenta.

Dos findings nuevos, **ninguno bloqueante y los dos rutados a SPEC-009**:
**F-SPEC-008-V39** (la exclusión de `docs/diseno/` es más ancha que su motivo) y
**F-SPEC-008-V40** (**sí hay una sexta lista de extensiones**, y es la única de
las seis que falla **abierto**).
...
> vitest run
 ...
 RUN  v4.1.11 /Users/albertofojo/src/tremen-dev/marcador.gal/.claude/worktrees/agent-a5f0e6094125c624c

 Test Files  81 passed (81)
      Tests  777 passed (777)
Type Errors  no errors
   Start at  22:28:18
   Duration  4.12s (transform 1.66s, setup 0ms, import 8.29s, tests 12.16s, environment 4ms, typecheck 141ms)
```
...
> node --env-file-if-exists=.env.local ./node_modules/vitest/vitest.mjs run --config vitest.integration.config.ts tests/db
 ...
 Test Files  8 passed (8)
      Tests  144 passed (144)
   Start at  22:28:26
   Duration  46.75s (transform 74ms, setup 0ms, import 273ms, tests 45.87s, environment 0ms)
```

Y las suites cerradas: `npx vitest run tests/mirror tests/site tests/docs` →
**48 ficheros / 347 casos**. `tests/polite` → **91/91**. Los cuatro números del
implementador se reproducen exactamente.

### Lo irreversible, primero

- **Ningún HTML real de terceros versionado.** `git diff --name-status main...HEAD`
  no añade **ni un** `.html`/`.htm`. `tests/fixtures/` son tres ficheros `.ts`
  sintéticos.
- **Ningún symlink versionado** (`git ls-files -s | awk '$1=="120000"'` → vacío).
- **`.env.local` no versionado**: `git ls-files | grep -i env` devuelve solo
  `.env.example`.
- **Ningún fichero de mutación superviviente**, ni en el índice ni en disco: ni
  `w7*`, ni `door.*`, ni `preflight*`, ni `side.ts`, ni `vprobe*`, ni `vdoor*`.
  Comprobado también bajo `robots/`, que `git status` no enseña.
- **Los tres `.gitkeep` siguen ahí**: `raw/.gitkeep`, `robots/.gitkeep` y
  `ventanas/jornada-1/robots/.gitkeep`. Al limpiar V35 borré **el fichero** y
  nunca el directorio, que es el aviso de la vuelta anterior.
- **`.gitignore` intacto**: su único diff contra `main` sigue siendo el
  `node_modules/` → `node_modules` de F-SPEC-008-V22. **La línea 17
  (`**/robots/*`) no se ha tocado**, que es lo que ADR-009 §3 hace irreversible.
- **`.oxlintrc.json` intacto** (diff vacío contra `main`): se **cita** como
  precedente, no se toca.

### El alcance del diff, comprobado y no supuesto

`git diff 834b5bd HEAD` (sexta verificación → HEAD) toca **cuatro ficheros**, y
de código solo dos: `tests/polite/architecture.test.ts` y
`tests/polite/support/capability.ts`. Los otros dos son la spec (solo el
frontmatter: `estado` e `historial`) y este ledger.

`git diff 9da82ed HEAD -- src migrations .gitignore .oxlintrc.json` está
**VACÍO**. **`src/` no tiene una línea de diff desde la enmienda**, así que la
evidencia de los CA ✅ se conserva entera.

### Ataqué F-SPEC-008-V37 por mi cuenta, y está cerrado

Escribí yo los ficheros, sin mirar los casos 2h/2i, y comparé cada uno contra su
gemelo `.ts`. Todos con `node:child_process`, que **no** es entrada de
`ALLOWED_PACKAGES`:

| Fichero | Resultado |
|---|---|
| `src/ingest/w7door.ts` (referencia) | **ROJO**: `architecture` **3** — `src/ingest/w7door.ts: node:child_process is not a declared package entry` — y **13** (huérfano) |
| `src/ingest/w7door.js` | **ROJO**: `architecture` **3** y **13**, **el mismo caso y el mismo mensaje**; lo único que cambia es el nombre del fichero |
| `src/site/w7widget.jsx` | **ROJO**: `architecture` **3** y **13**, ídem |
| `src/ingest/w7door.mjs` | **ROJO**: `architecture` **3** y **13**, ídem |
| `src/ingest/w7door.cjs` (con `require`) | **ROJO por CUATRO caminos**: **3** fallando cerrado (`the compiler names node:child_process and the reader did not enumerate it`), **3b**, **8** (CA-2.4, `src/ingest/w7door.cjs: require`) y **13** |
| `w7outside.js` + `.jsx` + `.mjs` + `.cjs` en la **raíz** | **ROJO**: `architecture` **1** — `expected [ 'w7outside.cjs', …(3) ] to deeply equal []`. La cobertura los echa de menos, que es la mitad que antes no existía |

**La ruta viva, reproducida entera y no de oídas.** Escribí
`src/app/(gl)/w7probe/route.js` con `import { execFileSync } from
'node:child_process'` y comprobé las dos mitades:

- `npx next build` la **compila** y la pone en la tabla de rutas de producción:
...
- `npx next start -p 3991` + `curl http://127.0.0.1:3991/w7probe` → **`w7probe`**.
  La ruta **se sirve y el `node:child_process` se ejecuta de verdad**. La premisa
  del finding era cierta.
- Y hoy ese mismo fichero es **ROJO** en `architecture` **3**
  (`src/app/(gl)/w7probe/route.js: node:child_process is not a declared package entry`)
  y en **17** (`ENTRY_POINTS` no la nombra: una ruta que ahora se lee es una ruta
  que hay que declarar).

Limpiado borrando **el fichero** y retirando el directorio con `rmdir`, nunca con
`rm -rf`.

**Y la declaración manda de verdad, no es decorado**: borrar `.js` de
`SCAN_EXTENSIONS` pone rojo **2e**, **2h** y **2i**.

### La exclusión gobierna las dos listas: no pude silenciar una sola

Cuatro mutaciones propias, cada una intentando desacoplar la exclusión de una de
las dos listas o esconder código con ella:

| # | Mutación | Resultado |
|---|---|---|
| **Y1** | `versionedSources()` **deja de aplicar** las exclusiones (silencia solo el escaneo) | **MUERE**: `architecture` **1** (`expected [ 'docs/diseno/_logic.js', …(1) ] to deeply equal []`) y **2h** |
| **Y2** | **Dos declaraciones otra vez**: `docs/diseno/` sale de `SCAN_EXCLUSIONS` y se escribe **a mano** dentro de `versionedSources()` | **MUERE**: `architecture` **2b** (`expected [ Array(2) ] to deeply equal [ 'node_modules/', 'docs/diseno/' ]`) y **2h** (`` `docs/diseno/` no está declarada como exclusión ``) |
| **Y3** | `src/ingest/` como exclusión declarada **con su motivo** —esconder código real, versionado y desplegado— | **MUERE por SIETE caminos**: `architecture` **2b**, **2d**, **2g**, **2i**, **13**, **14** y **17** |
| **Y4** | `src/db/` como exclusión declarada con su motivo (el directorio donde vivieron las evasiones 8ª y 9ª) | **MUERE**: `architecture` **2b** y **17** |

**No hay forma de mover una lista sin la otra**, y una exclusión no puede
esconder código real bajo una raíz sin poner rojo al menos un caso nombrado.

### El residuo escrito es cierto, y en un punto es más pequeño que el real

Reproduje los tres que la séptima vuelta declara abiertos:

- **F-SPEC-008-V34 — sigue abierto, exactamente como CA-2.4 lo escribe.**
  `src/ingest/w7v34.ts` con
  `const cp = process.getBuiltinModule('node:child_process');`, **enganchado** a
  `src/ingest/adapter.ts` para ser alcanzable desde `ENTRY_POINTS`:
  **`lint exit=0`, `npm test` 777/777, `tests/polite` 91/91**. Cierto.
- **F-SPEC-008-V35 — sigue abierto.** `robots/w7side.ts` en la raíz del
  repositorio: `git status` **no enseña nada**, `versionedSources()` no lo lista,
  `tests/polite` sigue en **91/91**. Cierto.
- **F-SPEC-008-V36 — sigue abierto y falla cerrado, y el ensanche lo agranda como
  la salvedad dice.** Un `.mjs` **legítimo y de verdad importado** —
  `src/ingest/w7legit.mjs` con `export { w7legit } from './w7legit.mjs'` en
  `adapter.ts`— pone rojo `architecture` **13** por huérfano, porque
  `resolveModule()` solo resuelve `.ts`/`.tsx`. **Rojo falso, nunca verde falso.**

**Y una cosa que el residuo escrito NO recoge, medida y dejada aquí:** ver
F-SPEC-008-V39 abajo. La medición del implementador (`versionedLost: []`, «no
mueve ni un fichero») es **cierta y honesta sobre el conjunto de ficheros de
hoy**, pero no es lo mismo que decir que la exclusión no cuesta cobertura: lo que
se pierde no es un fichero, es **un guardián**.

### Nada se ha tapado por la puerta de atrás

- **`CAPABILITY_NAMES` intacto.** Sigue siendo la lista de **nueve** nombres
  —`globalThis`, `fetch`, `XMLHttpRequest`, `WebSocket`, `EventSource`,
  `navigator`, `eval`, `Function`, `require`—. `git diff 9da82ed HEAD` sobre
  `capability.ts` no toca ni una línea suya.
- **`process` no está en ninguna lista** (`grep "'process'"` sobre
  `capability.ts` → cero). **ADR-016 §3.5 respetado.**
- **`versionedSources()` no se ha ensanchado por F-SPEC-008-V35**: lo único que
  se le añadió es aplicar las exclusiones declaradas, y por eso V35 se reproduce
  igual.
- **`resolveModule()` sin tocar** (F-SPEC-008-V36): sigue con su cuarta lista.
- **`ALLOWED_PACKAGES` sin tocar** en esta vuelta (`git diff 834b5bd HEAD` sobre
  `capability.ts` no cambia ni un `specifier:`).
- **`.gitignore`, `.oxlintrc.json`, `src/` y `migrations/` con diff vacío.**
- **El precedente que la exclusión cita es real y lo comprobé**: `.oxlintrc.json`
  lleva `/docs/diseno/` en `ignorePatterns`, y `5b632df` es
  *«Gate de calidad: oxlint ignora docs/diseno, que no es codigo»*, un commit de
  un fichero.

### N4 sigue sobreviviendo, y es lo que tiene que pasar

Reescribí el segundo parser de `robots.txt` **funcional** dentro de
`src/site/robots-txt.ts` —`w7SecondRobotsParser` y `w7SecondIsAllowed`, con
desempate por longitud y el `Allow` ganando el empate—:
`npx vitest run tests/polite tests/site` → **18 ficheros / 189 casos, todos
verdes**. Coincide con el número del implementador. **La pérdida firmada de
CA-2.8 (F-SPEC-008-20) sigue en pie y esta vuelta no la ha cerrado por
accidente.** Revertido.

### CA-7 y CA-14 siguen ciertos, comprobados rompiendo

- **CA-7.** `MIN_REQUEST_INTERVAL_MS` de `60_000` a `1_000` → **9 casos rojos en
  3 ficheros**: `adapter` 10, 11, 12, 13; `polite/rate-limit` 2 y 7; y
  `mirror/capture/rate-limit` 1, 2, 4. El número es load-bearing **en las dos
  mitades**. Mismo resultado que la cuarta, la quinta y la sexta verificación.
- **CA-14.7 — se falla cerrado.** `capture()` con `.catch(() => true)` sobre
  `takeTurn` → **ROJO** en `adapter` **19** («`capture()`: cero peticiones, cero
  bytes archivados y el error sin envolver»).
- **CA-14 durable.** El `where` de `src/db/rate-limit.ts` con `${now}` en vez de
  `${limit}` —conceder siempre— → **8 casos rojos** en `npm run test:db`: la
  batería de contrato 2, 5, 6, 7, más **CA-14.2** (3), **CA-14.4** (5 y 6) y
  **CA-14.6** (7). El caso 5 es la cláusula de no acumular que `/robot` publica en
...
sobre `tests/mirror tests/site tests/docs`: **44 ficheros en cada referencia** y
**una sola línea de diferencia**, `tests/mirror/user-agent.test.ts` **15 → 16**,
que **sube**. **Ningún caso borrado.** Sobre las suites de SPEC-001
(`tests/model tests/raw tests/db tests/types`), la única diferencia es el fichero
**nuevo** `tests/db/rate-limit.test.ts:7`. Corridas: **48 ficheros / 347 casos**
verdes. F-SPEC-008-1 sigue **arbitrada y no revisitada**.
...
Las 6 h de ADR-014 §3.2, la licencia de `competition_id: 'robots'`,
F-SPEC-008-1 (CA-3 ⚠️), F-SPEC-008-2 (CA-8 ⚠️), F-SPEC-008-V20 (CA-1 ⚠️),
F-SPEC-008-V3 (CA-6 ⚠️), el tamaño de `ALLOWED_PACKAGES` y **la pérdida firmada
de CA-2.8**: comprobados como registro, **no reabiertos**. Y **CA-2 tiene ⚠️ como
estado máximo alcanzable**: el verde de CA-2.6 no lo sube a ✅.

### Findings del verificador — séptima vuelta

Los dos son **no bloqueantes** y **los dos van a SPEC-009**, que es la spec de
dónde vive la declaración de qué es código. Ninguno reabre nada arbitrado y
ninguno toca `src/`.

- **F-SPEC-008-V39 — ⚠️ la exclusión de `docs/diseno/` es por DIRECTORIO, así que
  es más ancha que el motivo que la justifica. Es el agrandamiento medido de
  F-SPEC-008-V38, no un finding contra CA-2.6. Destino: SPEC-009.**
  El motivo escrito habla de dos ficheros —`_logic.js` y `build.mjs`, que son lo
  único de código que `git ls-files docs/diseno` lista—, pero la entrada excluye
  el **directorio**, y desde F-SPEC-008-V37 las exclusiones gobiernan también la
  **cobertura**. Consecuencia, y está **medida**: un `docs/diseno/w7side.ts` con
  `node:child_process` era **ROJO en `architecture` 1 sobre `834b5bd`**
  (`expected [ 'docs/diseno/w7side.ts' ] to deeply equal []`) y es **VERDE en
  HEAD**. Lo que se perdió no es un fichero —`versionedLost: []` es cierto sobre
  el árbol de hoy— sino **un guardián que ayer existía**, y la evidencia del
  ledger («no mueve ni un fichero») es un fotograma, no esa afirmación.
  **Y llega más lejos que la primera pata de la salvedad.** V38 dice que
  `docs/diseno/` «no se despliega (no cuelga de `src/app/` ni de ninguna raíz de
  Next)», y eso es cierto **del directorio, no de su contenido**: añadiendo
  `import { w7Side } from '../../../../docs/diseno/w7side'` a
  `src/app/(gl)/proxecto/page.tsx` —una ruta desplegada— y llamándolo en el
  cuerpo, `npx next build` da `✓ Compiled successfully` y el módulo **entra en el
  bundle de producción**, con `lint exit=0`, `npm test` **777/777**,
  `architecture` **41/41** y un `git add` normal. La **segunda** pata de la
  salvedad («nada de `src/` lo importa — y si algún día lo importara… quedaría
  igualmente sin leer») y su disparador («el día que algo bajo una raíz importe
  algo de `docs/diseno/`») **sí lo nombran exactamente**, y por eso esto agranda
  el residuo en vez de contradecirlo.
  **Por qué NO es bloqueante, dicho sin suavizar**: (1) **CA-2.6 lo autoriza
  expresamente** —*«lo que el criterio garantiza es que quedar fuera sea una
  decisión declarada en las raíces, no un efecto colateral de otra regla»*—, y
  aquí hay entrada, motivo sustantivo y precedente verificado (`5b632df`), que es
  justo lo que a JavaScript le faltaba en V37; (2) la **sexta verificación
  prescribió esta opción con estas palabras** («declarar `docs/diseno/` como
  exclusión con su motivo… o traerlo bajo una raíz»); (3) el residuo está escrito
  y rutado. Es la diferencia exacta entre V37 —un hueco **sin ninguna
  declaración**— y éste —**el precio declarado de una**—.
  **Cómo se cierra, y es un diff y no un arbitraje**: sustituir la entrada de
  directorio por las **dos rutas exactas** (`docs/diseno/_logic.js` y
  `docs/diseno/build.mjs`), cada una con su motivo. `excluded()` ya trata una
  entrada sin `/` final como coincidencia exacta, así que **no hace falta ningún
  mecanismo nuevo**; el precio es que un fichero nuevo del sistema de diseño pida
  su línea, que es la disciplina que el criterio quiere. **Qué lo despierta**: lo
  de V38, más el día que se firme el criterio de SPEC-009.

- **F-SPEC-008-V40 — ⚠️ SÍ hay una sexta lista de extensiones, y es la ÚNICA de
  las seis que falla ABIERTO. No es un hueco de RN-11 y no es un CA de esta spec.
  Destino: SPEC-009.**
  El encargo pedía buscarla, y la respuesta es que existe. Las seis, con su modo
  de fallo:
  1. `SCAN_EXTENSIONS` — **la declaración**. Ocho sufijos con motivo.
  2. `isCodeFile()` / `scannedSources()` — **derivada** de ella. Comprobado.
  3. `extensionPathspec()` / `versionedSources()` — **derivada** de ella.
     Comprobado (Y1, Y2).
  4. `resolveModule()` (`tests/mirror/support/imports.ts:539`, `.ts`/`.tsx`) —
     **F-SPEC-008-V36**, y **falla cerrado**: rojo falso en `architecture` 13,
     medido arriba.
  5. `tsconfig.json` — `include: ["**/*.ts", "**/*.tsx", …]` con
     `allowJs: false`. Es una lista de extensiones de hecho, pero **falla
     cerrado**: `read()` de `imports.ts` devuelve `unparseable: true` cuando
     ningún proyecto ve el fichero, y eso es rojo. Comprobado: mis `.js`, `.jsx`
     y `.mjs` los parsea y los juzga limpiamente; el `.cjs` cae por el camino de
     fallo cerrado. **No abre nada.**
  6. **`tests/site/no-hardcoded-literals.test.ts:44` y
     `tests/site/title-source.test.ts:40`**, que llevan cada una su propio
     `endsWith('.ts') || endsWith('.tsx')` sobre `src/app/` + `src/site/`. **Ésta
     falla ABIERTO.** Medido: `src/app/(gl)/w7jsx/page.jsx` con
     `title="Texto duro en castellano"` y `{'Resultados en directo'}` —dos
     infracciones de SPEC-004 CA-5 y de **D-2**— deja `tests/site` en **98/98
     verde**: el guardián de i18n no lo ve.
  **Lo que lo acota, y va dicho**: no es una vía de capacidad ni de RN-11 —el
  fichero no importa nada prohibido—, y **CA-2 sí lo caza a medias**: mientras no
  se declare en `ENTRY_POINTS`, `architecture` **17** se pone rojo
  (`expected […] to include 'src/app/(gl)/w7jsx/page.jsx'`), así que la ruta no
  entra en silencio. Lo que queda abierto es que, **una vez declarada** —un diff
  legítimo—, la mitad de i18n sigue ciega. **No es finding contra SPEC-008**:
  ningún CA de esta spec habla de los guardianes de `tests/site/`, y SPEC-004 y
  SPEC-006 están `hecho`. **Qué lo despierta**: el día que entre un `.jsx`, `.js`
  o `.mjs` de verdad bajo `src/app/` o `src/site/`.
...
- **Lo que NO es finding mío y pertenece a SPEC-009**, dicho aquí porque rutar
  ...
  **F-SPEC-008-V35** (la cobertura fuera de las raíces) y **F-SPEC-008-V36** (la
  ...
  los tres siguen abiertos **exactamente como su residuo los describe**. No son
  findings que reabrir: son alcance de otra spec, y su residuo está bien escrito.
  **F-SPEC-008-V38** sigue donde estaba, y **V39 lo agranda con la medida
  delante**.

### Lo que necesita decisión humana

1. **F-SPEC-008-7 sigue sin pedir**: el dictamen de `sdd-legal-datos` sobre
   `ceroacero.es` **en régimen de ingesta**. Sigue siendo precondición de correr
   la primera jornada, no de escribir código, y sigue siendo del humano. Es el
   único punto de esta spec que llega a `hecho` sin contestar.
2. **SPEC-009 tiene ahora seis entradas y una pregunta abierta**: V34, V35, V36,
   V38, **V39** y **V40**, más la pregunta de si CA-2.4 pasa a enumerar los
   identificadores globales permitidos (notas §16 de la spec). Su fecha real
   sigue siendo la que fijó la enmienda: **antes del día que se despliegue algo
   que le pida bytes a un tercero**, porque ese día la infracción latente de
   CA-2.4 pasa a consumada y lo que `/robot` publica en dos lenguas deja de ser
   cierto.

### Qué entrega esta spec y qué deja sin entregar

**Entrega**: el comodín `*` y el ancla `$` de `robots.txt` por fin respetados
—F-SPEC-002-23 cerrado, que era un incumplimiento vivo de RN-11 en `main`—; la
cortesía de RN-11 con **un solo dueño** en `src/polite/` (ADR-014 §1, sin fachada
de compatibilidad); el primer adaptador de `ceroacero.es`, que archiva antes de
parsear (RN-10), no publica (RN-08), no adivina identidad (RN-09) y produce
`Observation` deterministas y replicables desde el archivo; el registro de
fuentes que hace de una fuente una entrada de configuración y no una rama; y el
ritmo de 1 petición/minuto por par **que sobrevive al proceso**, en Postgres, con
concesión atómica —comprobada contra ocho procesos peleándose por el mismo turno—
y fallo cerrado.

**No entrega**, y por eso CA-2 no está en verde: **la frontera de capacidad
completa**. CA-2 cierra los `import` por superficie leídos con el compilador, la
contención en ejecución sobre el socket y la contención de conjuntos contra la
política real — pero **CA-2.4 está medido abierto** (`process.getBuiltinModule`
alcanza cualquier módulo interno sin ningún `import`, y su mecanismo es una lista
negra de nueve nombres, que es lo que ADR-016 §3.5 prohíbe), y **fuera de las
raíces la cobertura la sigue decidiendo `.gitignore`**. Eso es **SPEC-009** y una
firma humana, no un parche aquí. Tampoco entrega ninguna de las cuatro cifras de
EPIC-002, ni el cron, ni el calendario, ni el catálogo de alias, ni una sola
petición corrida contra `ceroacero.es`: **nada se ha ejecutado nunca contra la
fuente**, y esta verificación lo confirma por séptima vez.
