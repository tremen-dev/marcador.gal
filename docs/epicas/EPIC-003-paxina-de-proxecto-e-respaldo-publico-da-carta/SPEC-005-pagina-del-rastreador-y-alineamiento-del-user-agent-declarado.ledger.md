...
| CA-4 | `src/mirror/user-agent.ts` (`USER_AGENT_CONTACT = 'https://marcador.gal/robot'`) | `tests/mirror/user-agent.test.ts` casos 9-11: **literal** `https://marcador.gal/robot`, igualdad con `${SITE_ORIGIN}${CRAWLER_PATH.gl}` y literales para `SITE_ORIGIN` y `CRAWLER_PATH.gl`, y ya no es un `mailto:`. **La mitad de producción —que la URL responda `200` en el sitio desplegado— es del verificador** | Mitad de código verde: `USER_AGENT_CONTACT` es el literal `https://marcador.gal/robot` y además `=== ${SITE_ORIGIN}${CRAWLER_PATH.gl}`. **Mutación** devolviendo el `mailto:`: 10 casos en rojo en 5 ficheros, incluido `contact.test.ts` caso 2. **La mitad de producción NO se puede comprobar hoy**: `dig marcador.gal A` → `82.98.135.43` (aparcamiento de Dinahosting) y `curl -L https://marcador.gal/robot` → `000`, sin respuesta. La spec dice que esa comprobación «no es opcional», así que el CA queda **parcial**, no cerrado. **LA MITAD DE PRODUCCIÓN QUEDA COMPROBADA — 2026-09-01, `sdd-verificador`, contra el sitio desplegado.** `dig +short marcador.gal A` → `64.29.17.65` / `216.198.79.1` (Vercel); el aparcamiento de Dinahosting desapareció. **`curl` a la dirección EXACTA que lleva `USER_AGENT_CONTACT`, `https://marcador.gal/robot`: `FINAL=200`, `num_redirects=0`, `url_effective=https://marcador.gal/robot`, `ssl_verify_result=0`**, `content-type: text/html; charset=utf-8`, `x-matched-path: /robot`, 12686 bytes; `/es/robot` igual (`200`, `num_redirects=0`, `x-matched-path: /es/robot`). Ya no es `000`. Y la página que hay al otro lado es la que dice ser, comprobado sobre el HTML servido y en navegador real: contiene **una** aparición de la cadena `marcador.gal/0.0.1 (+https://marcador.gal/robot; medicion de latencia)` carácter a carácter (`grep -c -F` contra el valor importado de `src/mirror/user-agent.ts` → `1` en `/robot` y `1` en `/es/robot`, `0` en las dos de `/proxecto`), `<html lang="gl">` con `<title>O rastrexador — marcador.gal</title>` y `<html lang="es">` con `El rastreador — marcador.gal`, el `mailto:ola@tremen.dev` presente, y cero apariciones de `futgal|ceroacero|besoccer|resultados-futbol|rfgf` y cero de `SPEC-|RN-n|EPIC-|ADR-`. **El bucle URL→404 que F-SPEC-002-1 temía no existe: el `+` del user-agent lleva hoy a una página viva** | ✅ |
...
| CA-7 | `src/app/(gl)/robot/page.tsx`, `src/app/(es)/es/robot/page.tsx` | `tests/site/crawler-routes.test.ts` casos 1-5: `/robot` y `/es/robot` fijados con **literales**; existe el directorio de ruta de cada una; ninguna redirección las tiene por origen; el único comodín es el de `www` y sale del ápice; y la dirección del user-agent es la del ápice. **El `200` sin `3xx` sobre el despliegue es del verificador** (medido en local: ver Evidencia) | Local, sobre el build de producción: `curl -o /dev/null -L` da `num_redirects=0 code=200` para `/robot` y para `/es/robot`; control negativo, `/` da `308`. Casos 1-5 verdes. **La mitad de producción sigue pendiente**, misma causa que CA-4: el DNS no apunta. **CIERRA CONTRA PRODUCCIÓN — 2026-09-01, `sdd-verificador`.** Sobre `https://marcador.gal`, con `curl -sS -o /dev/null -w '%{http_code} %{num_redirects} %{url_effective}' -L --max-redirs 10`: **`/robot` → `200` con `num_redirects=0`** y `url_effective` idéntica a la pedida; **`/es/robot` → `200` con `num_redirects=0`**. El volcado de cabeceras completo lo confirma sin intermediarios: la **primera y única** respuesta de cada una es `HTTP/2 200` con `x-matched-path: /robot` y `/es/robot` respectivamente — **no hay ningún `3xx` delante, ni un `location:`, ni un `refresh:`**, que es lo que sí aparece en las respuestas de `/` y `/es`. Control negativo en la misma tanda, para que el `0` signifique algo: `/` da `308` con `location: /proxecto` y `num_redirects=1`, y `www.marcador.gal/` da `num_redirects=2`. La permanencia que pide ADR-010 se cumple hoy en el sitio real: la dirección que un tercero copie en su `robots.txt` responde a la primera | ✅ |
