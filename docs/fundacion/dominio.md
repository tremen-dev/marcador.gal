...
| **Radio Galega** | Emisora pública de la **CRTVG** que sigue el fútbol galego en directo con una persona en cada campo de Terceira. **Candidata a segunda fuente automática independiente** (EPIC-005). Su `SourceId` es `radio_galega`, con guion bajo, que la clave del raw store admite. | Añadido el 2026-09-12 (EPIC-005, ADR-028, **aprobado por Alberto Fojo el 2026-09-12**). **Categoría `broadcaster`, peso 0.7, derivada de máquina** (ver abajo). **Es capturable por consentimiento expreso del titular**, no por `robots.txt`: su aviso legal reserva el «tratamiento informático» de sus contenidos y, sin ese consentimiento, estaría bloqueada como `futgal.es` (ADR-008 §1). El alcance declarado está en `docs/legal/2026-09-12-consentimiento-crtvg-radio-galega.md`; la copia del documento sigue pendiente. Es **una fuente, no un programa**: otro programa de la misma emisora es la misma fuente; otra emisora es otra fila y otro consentimiento. **No entra en `DEFAULT_SOURCES`**: no se sondea, se escucha (ADR-029). |
| **`broadcaster`** (emisora transcrita) | **Categoría nueva de RN-01**, fijada por ADR-028 §1 (aprobado por Alberto Fojo el 2026-09-12) y escrita en `reglas.md` con esa fecha: una emisora con una persona en el campo cuyo audio se transcribe y se lee por máquina. **Peso 0.7**, el de un agregador, y **en el umbral de la segunda vía de RN-02 a propósito**: por debajo no podría corroborar a nadie. | Añadido el 2026-09-12 (EPIC-005, ADR-028). En código es una clave más de `RN01_WEIGHTS` (`src/ingest/sources.ts`) y un rol más de `SOURCE_ROLES` (`src/decide/roles.ts`), por ADR-011 §6. **Toda fuente de esta categoría es derivada de máquina** por cómo llega al motor, y por eso la propiedad es del rol y no de la fuente. |
| **derivada de máquina** (`machine_derived`) | Propiedad de una **categoría** de RN-01 cuya cadena entera hasta la `Observation` es máquina —audio → ASR → LLM— sin que ninguna persona mire el hecho antes de que exista la fila. **Su `Observation` existe y se escribe** (RN-13), pero **nunca sostiene sola una `Decision`, ni provisional**: no es la observación que manda, no abre `scheduled → live`, no publica marcador. **Suma y no resta**: cuenta como segunda fuente donde las reglas piden una (RN-04, RN-06) y como segunda independiente de RN-02 solo con el par declarado (ADR-021 §7); una discrepancia con ella **alerta** (RN-05) pero no retiene; y **no acalla** el silencio de RN-07. | Añadido el 2026-09-12 (EPIC-005, ADR-028 §2, **aprobado por Alberto Fojo el 2026-09-12**: reinterpreta D-4 en un punto, y la aclaración de RN-09 con ese texto está en `reglas.md` desde esa fecha; en el mismo gate se rechazó el veto en RN-05 —alerta sin retención— y se ratificó que no acalla RN-07). Es **identidad, no número**: el reducer la lee de la tabla de roles con `isMachineDerived(role)`, que falla cerrado como `isHuman` e `isOfficial` (ADR-021 §8.4); el peso sigue siendo el congelado en la `Observation` (RN-01, aclaración del 2026-09-02). Hoy la única categoría así es `broadcaster`. La confirmación humana de RN-09 se cumple, para una fuente así, en el **catálogo de nombres orales** y en que la máquina no puede publicar sola. |
| **nombre oral** | Cómo **dice** un locutor el nombre de un equipo en antena («o Arenteiro», «Alondras de Cangas»), tal como lo transcribe el ASR. Es un **alias** de la fuente `radio_galega` en el sentido de ADR-018: enrutado vigente, estable y finito, declarado por una persona en `alias/<temporada>/radio_galega.json` y cargado con el cargador de SPEC-011 **sin tocar el mecanismo**. | Añadido el 2026-09-12 (EPIC-005, ADR-028 §8). **Aquí el catálogo de alias sí encaja, y con el corresponsal no** (ADR-022 §5): las grafías de una emisora son estables; las de una persona escribiendo desde una banda, no. El extractor rechaza (`unaliased_team`) toda propuesta cuya cita no contenga un nombre oral declarado de **cada** equipo del partido elegido: así RN-09 —«sin alias confirmado por una persona»— se cumple por el dato y no por el criterio del modelo. La comparación tolera lo que la oralidad y el ASR no fijan —mayúsculas y acentos— como **divergencia declarada** respecto a SPEC-001 CA-5, no como cambio de `normalizeAlias`. Un nombre no declarado es **cobertura degradada por catálogo**. |
| **oyente** | La pieza que **escucha** el stream de una emisora dentro de una invocación larga de Vercel (`/api/cron/radio`, cada 10 min, `maxDuration` 800, 11 min de escucha con 1 de solape), corta el audio en **trozos** de 20–30 s siguiendo las fronteras de los segmentos HLS, y **archiva cada segmento tal como lo sirve el CDN antes de tocarlo** (RN-10). Su domicilio es `src/radio/`. | Añadido el 2026-09-12 (EPIC-005, ADR-029, **aprobado por Alberto Fojo el 2026-09-12**). **Nace inerte**: sin partidos en ventana de una jornada declarada (ADR-019 §2 y §3) vuelve sin abrir el stream, como el bot de ADR-022 §7. **No publica**: hereda la frontera de SPEC-008 CA-12 —no importa `DecisionStore` ni construye `Decision`— y termina en `Observation` persistida; quien lo llama y pasa el motor tras cada trozo es un ciclo hermano de `cycle.ts` en `src/decide/` (ADR-029 §4). **No es un proceso vivo**: nada de una invocación sobrevive en memoria a la siguiente; lo que dos invocaciones solapadas comparten está en `radio_chunks` y en el raw store. Sale por `politeFetch` a la cadencia que el stream declara (ADR-028 §9). |
| **extractor de goles** | El LLM que, por cada trozo transcrito, **propone** cero, una o varias observaciones **dentro de la lista cerrada de partidos en ventana** (ADR-022 §5), con salida zod y rechazos enumerados, y **solo con marcador explícito en la cita literal** de la transcripción: sin marcador dicho no hay `Observation`, y **no se deriva del estado**. Solo propone `live` o `finished`. | Añadido el 2026-09-12 (EPIC-005, ADR-028 §8). Va detrás de un puerto propio de `src/radio/` (patrón ADR-022 §6), con su DPA en `docs/legal/` antes de su adaptador. Sus rechazos son los cinco de `src/bot/proposal.ts` más tres: `evidence_not_in_transcript`, `no_explicit_score` y `unaliased_team`. **No tiene memoria entre trozos.** No confundir con el **extractor** de `src/mirror/analysis/` (lee para comparar fuentes) ni con la **propuesta** del bot (que una persona confirma con un botón; aquí nadie confirma un gol porque la máquina no puede publicarlo sola). |
